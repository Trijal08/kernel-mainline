...
/*
 * NOTE(mainline): the tz->ops->get_trip_type()/get_trip_temp()/get_trip_hyst()
 * callbacks and of_thermal_get_ntrips() were removed. Since this driver
 * includes the thermal core's internal header, trip points can be read
 * directly from the zone's thermal_trip_desc array (tz->trips[i].trip). These
 * helpers preserve the old callback semantics (returning 0 / -EINVAL) so the
 * call sites need only minimal changes.
 */
static inline int gs_tmu_get_trip_type(struct thermal_zone_device *tz, int trip,
				       enum thermal_trip_type *type)
{
	if (trip < 0 || trip >= tz->num_trips)
		return -EINVAL;
	*type = tz->trips[trip].trip.type;
	return 0;
}

static inline int gs_tmu_get_trip_temp(struct thermal_zone_device *tz, int trip,
				       int *temp)
{
	if (trip < 0 || trip >= tz->num_trips)
		return -EINVAL;
	*temp = tz->trips[trip].trip.temperature;
	return 0;
}

/*
 * NOTE(mainline): the thermal zone no longer keeps a single tz->thermal_instances
 * list and thermal_instance no longer stores a trip index (it now points at a
 * const struct thermal_trip). Cooling instances now hang off each trip
 * descriptor. This helper returns the first power-actor cooling instance bound
 * to the trip at @trip_idx, replacing the old
 * "for each instance: instance->trip == trip_idx && power_actor" loops.
 * The caller is responsible for holding tz->lock where required.
 */
static struct thermal_instance *
gs_tmu_get_power_actor_instance(struct thermal_zone_device *tz, int trip_idx)
{
	struct thermal_instance *instance;

	if (trip_idx < 0 || trip_idx >= tz->num_trips)
		return NULL;

	list_for_each_entry(instance, &tz->trips[trip_idx].thermal_instances, trip_node) {
...
/*
 * NOTE(mainline): retained (marked __maybe_unused) although its only consumer
 * was the dropped "clock_set_rate" systrace marker; kept for merge-friendliness.
 */
static const char * const __maybe_unused trace_suffix[] = {
...
{
	/*
	 * NOTE(mainline): this published the per-feature value as the
	 * Android-only "clock_set_rate" systrace counter (clock event class
	 * removed from mainline <trace/events/power.h>). No mainline
	 * equivalent; the marker is dropped.
	 */
}
...
static u8 __maybe_unused get_therm_press(struct gs_tmu_data *data)
{
...
/*
 * NOTE(mainline): gs_tmu_throt_freq() was consumed by an Android-only CPUfreq
 * vendor hook that has no mainline equivalent, so it currently has no caller.
 * It is kept (marked __maybe_unused) rather than deleted to stay merge-friendly
 * and is now static to avoid the missing-prototype warning.
 */
static unsigned int __maybe_unused gs_tmu_throt_freq(int cpu)
{
...
static int find_rotated_idx(struct gov_trace_data_struct *bulk_trace_buffer, int length)
{
	int left = 0;
	int right = length - 1;
...
static int next_greater_rotated_idx(struct gov_trace_data_struct *bulk_trace_buffer, int array_len,
...
static void update_tj_power_table_ect_offset(struct gs_tmu_data *data)
{
	int ect_offset;
...
static int gs_tmu_initialize(struct platform_device *pdev)
{
	...
	/*
	 * NOTE(mainline): of_thermal_get_ntrips() and the tz->ops->get_trip_*
	 * callbacks were removed. Trip points are now described by the zone's
	 * thermal_trip_desc array (tz->trips[i].trip), reachable through the
	 * thermal core's internal header. Read type/temperature/hysteresis
	 * directly from there, preserving the trip-index based programming.
	 */
	for (i = (tz->num_trips - 1); i >= 0; i--) {
		const struct thermal_trip *trip = &tz->trips[i].trip;
...
/*
 * NOTE(mainline): the .get_trend op now receives the trip directly
 * (const struct thermal_trip *) instead of a trip index.
 */
...
/*
 * NOTE(mainline): the .set_trip_temp op now receives the affected trip directly
 * (const struct thermal_trip *) instead of a trip index. Identify the trip's
 * index within the zone's trip array to know which threshold slot to override,
 * and read the remaining thresholds straight from tz->trips[].
 */
static int gs_tmu_set_trip_temp(struct thermal_zone_device *tz,
				const struct thermal_trip *trip, int temp)
{
	struct gs_tmu_data *data = tz->devdata;
	int i, trip_idx, ret = 0;
	unsigned char threshold[8] = {0, };

	trip_idx = -1;
	for (i = 0; i < tz->num_trips; i++) {
		if (&tz->trips[i].trip == trip) {
			trip_idx = i;
			break;
		}
	}
	if (trip_idx < 0) {
		pr_err("Failed to find trip in zone\n");
		return -EINVAL;
	}
	...
	for (i = (tz->num_trips - 1); i >= 0; i--) {
		if (i == trip_idx)
			...
			threshold[i] = (unsigned char)(tz->trips[i].trip.temperature /
						       MCELSIUS);
	}
	mutex_lock(&data->lock);
	if (data->enabled) {
...
static void get_control_trips(struct gs_tmu_data *data)
{
	...
	for (i = 0; i < tz->num_trips; i++) {
		...
		ret = gs_tmu_get_trip_type(tz, i, &type);
		if (ret) {
			dev_warn(&tz->device,
...
static void allow_maximum_power(struct gs_tmu_data *data)
{
	...
	mutex_lock(&tz->lock);
	/*
	 * NOTE(mainline): iterate only the cooling instances bound to the
	 * control-temp trip (instance->trip is now a pointer, and instances hang
	 * off the trip descriptor rather than a per-zone list).
	 */
	if (control_temp >= 0 && control_temp < tz->num_trips) {
		list_for_each_entry(instance, &tz->trips[control_temp].thermal_instances,
				    trip_node) {
			if (!cdev_is_power_actor(instance->cdev))
				continue;
			...
			/*
			 * NOTE(mainline): thermal_cooling_device.type is now
			 * const char *; this tracepoint takes char *. Cast away
			 * const at the call site (the trace only reads it).
			 */
			trace_thermal_exynos_allow_max_power(data->tmu_name,
							     data->is_hardlimited,
							     (char *)instance->cdev->type,
							     instance->target);
			/*
			 * NOTE(mainline): thermal_cdev_update() is not exported
			 * to modules. The thermal core re-evaluates and applies
			 * cooling-device states on its own governor passes, so
			 * pushing the update directly is dropped; instance->target
			 * is already set above for the core to consume.
			 */
		}
	}
...
static int gs_pi_controller(struct gs_tmu_data *data, int control_temp)
{
	...
	mutex_unlock(&data->lock);
	mutex_lock(&tz->lock);
	instance = gs_tmu_get_power_actor_instance(tz, data->trip_control_temp);
	if (instance) {
		found_actor = true;
		cdev = instance->cdev;
	}
	...
	/* NOTE(mainline): thermal_cdev_update() unexported; core applies cdev state. */
	mutex_unlock(&tz->lock);
	mutex_lock(&data->lock);
	data->max_cdev = state;
...
static void gs_pi_thermal(struct gs_tmu_data *data)
{
	...
	ret = gs_tmu_get_trip_temp(tz, data->trip_switch_on,
				   &switch_on_temp);
	...
	ret = gs_tmu_get_trip_temp(tz, data->trip_control_temp,
				   &control_temp);
	if (ret) {
...
static void gs_throttle_hard_limit(struct kthread_work *work)
{
	...
	/*
	 * NOTE(mainline): the thermal zone no longer keeps a single
	 * thermal_instances list (and thermal_instance lost its ->tz back
	 * pointer). Instances now hang off each trip descriptor's
	 * thermal_instances list. Iterate the zone's trips to find a bound
	 * cooling device; every instance reached this way belongs to this zone,
	 * so the original tmu_name/zone-type match is equivalent to checking the
	 * zone type once.
	 */
	mutex_lock(&tz->lock);
	if (!strncmp(data->tmu_name, tz->type, THERMAL_NAME_LENGTH)) {
...
static int gs_tmu_irq_work_init(struct platform_device *pdev)
{
	...
	/*
	 * NOTE(mainline): sched_setscheduler_nocheck() is not exported to
	 * modules; sched_set_fifo() is the exported helper. It assigns a
	 * mid-range SCHED_FIFO priority (MAX_RT_PRIO/2) rather than the
	 * original MAX_RT_PRIO-1, and cannot fail (returns void).
	 */
	...
	if (data->hotplug_enable || data->pause_enable) {
		...
		*/
		...
		/* NOTE(mainline): see sched_set_fifo() note above. */
		sched_set_fifo(thread);
		wake_up_process(thread);
	}
				    ...
				    size_t count)
{
	...
	for (i = 0; i < TRIP_LEVEL_NUM; i++) {
		...
		ret = gs_tmu_get_trip_temp(tz, i, &trip_temp);
		if (ret) {
			dev_err(&pdev->dev,
		...
		/*
		 * NOTE(mainline): the legacy tz->ops->set_trip_temp() callback no
		 * longer exists and trip points live in the thermal core's
		 * thermal_trip_desc array. As before (b/315931658) the trip
		 * temperature is written directly; the subsequent device update
		 * re-evaluates the zone with the new threshold.
		 */
		if (tz->trips)
			tz->trips[i].trip.temperature = new_trip_temp;
	}
...
static int param_update_acpm_pi_table_set(const char *val, const struct kernel_param *kp)
{
	...
	list_for_each_entry (data, &dtm_dev_list, node) {
		if (data->use_pi_thermal) {
			...
			instance = gs_tmu_get_power_actor_instance(tz,
...
static int gs_tmu_parse_ect(struct gs_tmu_data *data)
{
	...
	if (!data->use_pi_thermal) {
		...
		for (i = 0; i < function->num_of_range; ++i) {
			...
			/*
			 * NOTE(mainline): tz->ops->set_trip_temp() was removed;
			 * write the ECT-derived trip temperature straight into the
			 * thermal core's trip array (this runs at init before the
			 * zone is actively managed).
			 */
			if (i < tz->num_trips)
				tz->trips[i].trip.temperature = temperature * MCELSIUS;
		...
		for (i = 0; i < pidtm_block->num_of_temperature; ++i) {
			...
			/* NOTE(mainline): see above; set_trip_temp() removed. */
			if (i < tz->num_trips)
				tz->trips[i].trip.temperature = temperature * MCELSIUS;
		...
		if (value != -1) {
			...
			/* NOTE(mainline): tz->ops->set_trip_temp() removed; write trip 3 directly. */
			if (tz->trips && tz->num_trips > 3)
				tz->trips[3].trip.temperature = temperature * MCELSIUS;
...
/*
 * NOTE(mainline): register_tz_id_ignore_genl() is provided by the
 * VH_THERMAL vendor-hook module, which is disabled in this mainline port.
 * Gate the declaration and its (only) call site on CONFIG_VH_THERMAL. It
 * only added the zone to a thermal-genl notification ignore-list, which is
 * non-essential to TMU operation.
 */
...
/*
 * NOTE(mainline): platform_driver::remove now returns void, so it can no longer
 * refuse unbinding by returning -EBUSY as before. The driver was effectively
 * never removable; the body remains a no-op.
 */
