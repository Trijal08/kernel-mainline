				...
				unsigned long state)
{
	...
	/*
	 * NOTE(mainline): the Android-only "clock_set_rate" systrace counter
	 * (clock event class removed from mainline <trace/events/power.h>) is
	 * dropped; the vendor cdev trace above is retained.
	 */
...
/*
 * NOTE(mainline): struct thermal_instance lost its zone back-pointer (instances
 * are now anchored to a specific trip) and get_thermal_instance() was removed.
 * Match the cdev<->trip instance by comparing the trip pointer of the zone's
 * trip[index] against the instances hanging off the cooling device.
 */
static struct thermal_instance *gpu_cooling_get_instance(struct thermal_cooling_device *cdev,
	...
	struct thermal_cooling_device *cdev, char *cooling_name)
{
	...
	for (i = 0; i < function->num_of_range; ++i) {
		...
		instance = gpu_cooling_get_instance(cdev, tz, i);
		if (!instance) {
		...
		const char *buf, size_t count)
{
	...
	cdev->updated = false;
	mutex_unlock(&cdev->lock);
	/* NOTE(mainline): thermal_cdev_update() is not exported to modules; the
	 * thermal core re-evaluates the cooling device on its next governor pass. */
	return count;
}
