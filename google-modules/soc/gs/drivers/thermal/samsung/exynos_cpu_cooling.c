				 ...
				 unsigned long state)
{
	...
	if (ret == 1) {
		ret = 0;
		/*
		 * NOTE(mainline): the Android-only "clock_set_rate" systrace
		 * counter (clock event class removed from mainline
		 * <trace/events/power.h>) is dropped; the vendor cdev trace is
		 * retained.
		 */
...
/*
 * NOTE(mainline): struct thermal_instance lost its back-pointer to the thermal
 * zone (instances are now anchored to a specific trip), and get_thermal_instance()
 * was removed. Find the cdev<->trip instance by matching the trip pointer of the
 * zone's trip[index] against the instances hanging off the cooling device.
 */
static struct thermal_instance *exynos_cpu_get_instance(struct thermal_cooling_device *cdev,
							struct thermal_zone_device *tz,
							int trip_index)
{
	struct thermal_instance *instance, *found = NULL;
	...
	struct thermal_cooling_device *cdev, char *cooling_name)
{
	...
	/*
	 * NOTE(mainline): the cooling device no longer exposes the bound zone
	 * via its instances, so resolve the zone by name directly.
	 */
	...
	for (i = 0; i < function->num_of_range; ++i) {
		...
		instance = exynos_cpu_get_instance(cdev, tz, i);
		if (!instance) {
		...
		const char *buf, size_t count)
{
	...
	cdev->updated = false;
	mutex_unlock(&cdev->lock);
	/* NOTE(mainline): thermal_cdev_update() not exported to modules; core updates it. */
	return count;
}
			      ...
			      struct cpufreq_policy *policy, u32 capacitance, char *cooling_name)
{
	...
	/* NOTE(mainline): ida_simple_get() -> ida_alloc(). */
...
remove_ida:
	/* NOTE(mainline): ida_simple_remove() -> ida_free(). */
