...
/*
 * NOTE(mainline): these are EXPORT_SYMBOL_GPL'd public APIs but the shared
 * header (soc/google/isp_cooling.h) does not declare them, which trips
 * -Werror=missing-prototypes. Declare them here so the definitions have a
 * visible prototype; the canonical declarations belong in that header.
 */
...
struct thermal_cooling_device *isp_cooling_register(const struct cpumask *clip_isp);
struct thermal_cooling_device *of_isp_cooling_register(struct device_node *np,
 ...
 */
static struct thermal_instance *isp_cooling_get_instance(struct thermal_cooling_device *cdev,
				   ...
				   char *cooling_name)
{
	...
	for (i = 0; i < function->num_of_range; ++i) {
		...
		instance = isp_cooling_get_instance(cdev, tz, i);
		if (!instance) {
