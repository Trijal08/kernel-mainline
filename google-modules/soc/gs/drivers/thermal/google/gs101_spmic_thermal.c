 ...
 * of_thermal_is_trip_valid() were removed. Read trips directly from the thermal
 * core's trip array (available via the included internal header) and pick the
 * HOT trip's temperature.
 */
static int gs101_spmic_thermal_get_hot_temp(struct thermal_zone_device *tzd)
{
	...
	for (i = 0; i < tzd->num_trips; i++) {
		const struct thermal_trip *trip = &tzd->trips[i].trip;
