...
/*
 * NOTE(mainline): struct em_perf_domain no longer exposes a directly indexable
 * "table" member; the performance state table is now reached through
 * em_perf_state_from_pd() under the RCU read lock. These helpers wrap that
 * access and return a single field value.
 */
static unsigned long uclamp_em_frequency(struct em_perf_domain *em, int idx)
{
...
static unsigned long uclamp_em_power(struct em_perf_domain *em, int idx)
{
	struct em_perf_state *table;
	unsigned long power;
					...
					unsigned long state)
{
	...
	if (state != uclamp_cdev->cur_state) {
		...
		pr_debug("cdev:[%s] new state request:[%lu] frequeny:[%lu]\n",
			 cdev->type, state,
			 uclamp_em_frequency(uclamp_cdev->em, idx));
				    ...
				    u32 power)
{
	...
	for (idx = 0; idx < uclamp_cdev->max_state; idx++) {
		if (uclamp_em_power(uclamp_cdev->em, idx) >= power)
			break;
	}
					   ...
					   unsigned long freq)
{
	...
	for (idx = 0; idx < uclamp_cdev->max_state; idx++) {
		if (uclamp_em_frequency(uclamp_cdev->em, idx) >= freq)
			break;
	}
				      ...
				      unsigned long state, u32 *power)
{
	...
	idx = uclamp_cdev->max_state - state;
	*power = uclamp_em_power(uclamp_cdev->em, idx) * uclamp_cdev->related_cpu_cnt;
...
static void thermal_uclamp_cleanup(void)
{
	...
	list_for_each_entry_safe(uclamp_cdev, n, &therm_uclamp_cdev_list, cdev_list) {
		...
		sched_thermal_freq_cap(
				uclamp_cdev->cpu,
				uclamp_em_frequency(uclamp_cdev->em,
						    uclamp_cdev->max_state));
		kfree(uclamp_cdev);
	}
	mutex_unlock(&therm_cdev_list_lock);
}
