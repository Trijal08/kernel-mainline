...
{
	struct s2mpg15_spmic_thermal_sensor *s = thermal_zone_device_priv(tz);
	struct s2mpg15_spmic_thermal_chip *s2mpg15_spmic_thermal = s->chip;
	int raw, ret = 0;
	u8 mask = 0x1;
	u8 data_buf[S2MPG15_METER_NTC_BUF];
	...
	*temp = s2mpg15_map_volt_temp(raw);
	...
	if (s2mpg15_spmic_thermal->stats_en & (mask << s->adc_chan)) {
		int metrics_ret = temp_residency_stats_update(s->tr_handle, *temp);
 ...
 * (const struct thermal_trip *) instead of a trip index, so the old
 * of_thermal_get_trip_points() lookup is no longer needed.
 */
static int s2mpg15_spmic_thermal_set_trip_temp(struct thermal_zone_device *tz,
...
{
	struct s2mpg15_spmic_thermal_sensor *sensor = thermal_zone_device_priv(tz);
	int ret = 0;
	u8 value = 0, mask = 0x1;
...
tz_temp_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	/* NOTE(mainline): tzd->temperature is private; read via accessor. */
...
/*
 * NOTE(mainline): of_thermal_get_ntrips()/of_thermal_get_trip_points()/
 * of_thermal_is_trip_valid() were removed. Walk the zone's trips via the
 * generic thermal_zone_for_each_trip() iterator and pick the HOT trip's
 * temperature instead.
 */
static int s2mpg15_spmic_thermal_hot_trip_cb(struct thermal_trip *trip, void *data)
{
	int *hot_temp = data;

	if (trip->type == THERMAL_TRIP_HOT &&
	    trip->temperature != THERMAL_TEMP_INVALID) {
		*hot_temp = trip->temperature;
		/* Stop the iteration once the HOT trip is found. */
		return 1;
	}
...
{
	int hot_temp = THERMAL_TEMP_INVALID;

	thermal_zone_for_each_trip(tzd, s2mpg15_spmic_thermal_hot_trip_cb, &hot_temp);
...
static void s2mpg15_spmic_thermal_remove(struct platform_device *pdev)
{
	int i;
