...
/*
 * NOTE(mainline): platform_device no longer has a writable driver_override
 * field (it moved into struct device and is now an opaque, locked structure).
 * It must be set at runtime via device_set_driver_override() before the device
 * is registered; see gnss_probe().
 */
...
static int gnss_probe(struct platform_device *pdev)
{
	...
	/* NOTE(mainline): set driver_override at runtime (see sscd_dev above). */
