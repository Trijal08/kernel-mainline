...
static const char ect_signature[] = "PARA";
				 ...
				 struct ect_dvfs_domain *domain)
{
	...
	if (domain->mode == e_dvfs_mode_sfr_address) {
		...
		domain->list_clock =
			kcalloc(domain->num_of_clock, sizeof(char *),
				GFP_KERNEL);
		domain->list_sfr = NULL;
		if (!domain->list_clock) {
			ret = -ENOMEM;
...
static int ect_parse_dvfs_header(void *address, struct ect_info *info)
{
	...
	ect_dvfs_header = kzalloc(sizeof(*ect_dvfs_header), GFP_KERNEL);
	if (!ect_dvfs_header)
		return -ENOMEM;
	...
	ect_dvfs_header->domain_list =
		kcalloc(ect_dvfs_header->num_of_domain,
			sizeof(struct ect_dvfs_domain),	GFP_KERNEL);
	if (!ect_dvfs_header->domain_list) {
		ret = -EINVAL;
...
static int ect_parse_pll_header(void *address, struct ect_info *info)
{
	...
	ect_pll_header = kzalloc(sizeof(*ect_pll_header), GFP_KERNEL);
	if (!ect_pll_header)
		return -ENOMEM;
	...
	ect_pll_header->pll_list = kcalloc(ect_pll_header->num_of_pll,
					   sizeof(struct ect_pll), GFP_KERNEL);
	if (!ect_pll_header->pll_list) {
		ret = -ENOMEM;
				    ...
				    struct ect_voltage_domain *domain)
{
	...
	domain->table_list =
		kcalloc(domain->num_of_table, sizeof(struct ect_voltage_table),
			GFP_KERNEL);
	if (!domain->table_list) {
		ret = -ENOMEM;
...
static int ect_parse_voltage_header(void *address, struct ect_info *info)
{
	...
	ect_voltage_header = kzalloc(sizeof(*ect_voltage_header), GFP_KERNEL);

	if (!ect_voltage_header)
		return -EINVAL;
	...
	ect_voltage_header->domain_list =
		kcalloc(ect_voltage_header->num_of_domain,
			sizeof(struct ect_voltage_domain), GFP_KERNEL);

	if (!ect_voltage_header->domain_list) {
		ret = -ENOMEM;
		goto err_domain_list_allocation;
	}
				...
				struct ect_rcc_domain *domain)
{
	...
	domain->table_list = kcalloc(domain->num_of_table,
				     sizeof(struct ect_rcc_table), GFP_KERNEL);
...
static int ect_parse_rcc_header(void *address, struct ect_info *info)
{
	...
	ect_rcc_header = kzalloc(sizeof(*ect_rcc_header), GFP_KERNEL);
	...
	ect_rcc_header->domain_list = kcalloc(ect_rcc_header->num_of_domain,
					      sizeof(struct ect_rcc_domain),
					      GFP_KERNEL);

	if (!ect_rcc_header->domain_list) {
		ret = -ENOMEM;
		goto err_domain_list_allocation;
	}
...
static int ect_parse_mif_thermal_header(void *address, struct ect_info *info)
{
	...
	ect_mif_thermal_header = kzalloc(sizeof(*ect_mif_thermal_header),
					 GFP_KERNEL);

	if (!ect_mif_thermal_header)
		return -EINVAL;
...
static int ect_parse_ap_thermal_function(int parser_version, void *address,
					 struct ect_ap_thermal_function *function)
{
	...
	function->range_list = kcalloc(function->num_of_range,
				       sizeof(struct ect_ap_thermal_range),
				       GFP_KERNEL);
...
static int ect_parse_ap_thermal_header(void *address, struct ect_info *info)
{
	...
	ect_ap_thermal_header =	kzalloc(sizeof(*ect_ap_thermal_header),
					GFP_KERNEL);

	if (!ect_ap_thermal_header)
		return -EINVAL;
	...
	ect_ap_thermal_header->function_list =
		kcalloc(ect_ap_thermal_header->num_of_function,
			sizeof(struct ect_ap_thermal_function), GFP_KERNEL);

	if (!ect_ap_thermal_header->function_list) {
		ret = -ENOMEM;
	...
	for (i = 0; i < ect_ap_thermal_header->num_of_function; ++i) {
		...
		ect_ap_thermal_function =
			&ect_ap_thermal_header->function_list[i];
...
static int ect_parse_margin_header(void *address, struct ect_info *info)
{
	...
	ect_margin_header = kzalloc(sizeof(*ect_margin_header), GFP_KERNEL);

	if (!ect_margin_header)
		return -EINVAL;
	...
	ect_margin_header->domain_list =
		kcalloc(ect_margin_header->num_of_domain,
			sizeof(struct ect_margin_domain), GFP_KERNEL);

	if (!ect_margin_header->domain_list) {
		ret = -ENOMEM;
		goto err_domain_list_allocation;
	}
...
static int ect_parse_timing_param_header(void *address, struct ect_info *info)
{
	...
	ect_timing_param_header = kzalloc(sizeof(*ect_timing_param_header),
					  GFP_KERNEL);

	if (!ect_timing_param_header)
		return -ENOMEM;
	...
	ect_timing_param_header->size_list =
		kcalloc(ect_timing_param_header->num_of_size,
			sizeof(struct ect_timing_param_size), GFP_KERNEL);

	if (!ect_timing_param_header->size_list) {
		ret = -ENOMEM;
...
static int ect_parse_minlock_header(void *address, struct ect_info *info)
{
	...
	ect_minlock_header = kzalloc(sizeof(*ect_minlock_header), GFP_KERNEL);

	if (!ect_minlock_header)
		return -ENOMEM;
	...
	ect_minlock_header->domain_list =
		kcalloc(ect_minlock_header->num_of_domain,
			sizeof(struct ect_minlock_domain), GFP_KERNEL);

	if (!ect_minlock_header->domain_list) {
		ret = -ENOMEM;
		goto err_domain_list_allocation;
	}
...
static int ect_parse_gen_param_header(void *address, struct ect_info *info)
{
	...
	ect_gen_param_header = kzalloc(sizeof(*ect_gen_param_header),
				       GFP_KERNEL);

	if (!ect_gen_param_header)
		return -ENOMEM;
	...
	ect_gen_param_header->table_list =
		kcalloc(ect_gen_param_header->num_of_table,
			sizeof(struct ect_gen_param_table), GFP_KERNEL);

	if (!ect_gen_param_header->table_list) {
		ret = -ENOMEM;
		goto err_table_list_allocation;
	}
...
static int ect_parse_bin_header(void *address, struct ect_info *info)
{
	...
	ect_bin_header = kzalloc(sizeof(*ect_bin_header), GFP_KERNEL);
	if (!ect_bin_header)
		return -2;
	...
	ect_bin_header->binary_list = kcalloc(ect_bin_header->num_of_binary,
					      sizeof(struct ect_bin),
					      GFP_KERNEL);

	if (!ect_bin_header->binary_list) {
		ret = -ENOMEM;
...
static int ect_parse_new_timing_param_size(int parser_version, void *address,
...
static int ect_parse_new_timing_param_header(void *address, struct ect_info *info)
{
	...
	ect_new_timing_param_header =
		kzalloc(sizeof(*ect_new_timing_param_header), GFP_KERNEL);

	if (!ect_new_timing_param_header)
		return -ENOMEM;
	...
	ect_new_timing_param_header->size_list =
		kcalloc(ect_new_timing_param_header->num_of_size,
			sizeof(struct ect_new_timing_param_size), GFP_KERNEL);

	if (!ect_new_timing_param_header->size_list) {
		ret = -ENOMEM;
		goto err_size_list_allocation;
	}
	...
	for (i = 0; i < ect_new_timing_param_header->num_of_size; ++i) {
		ect_new_timing_param_size =
			&ect_new_timing_param_header->size_list[i];

		ect_parse_integer64(&address,
				    &ect_new_timing_param_size->parameter_key);
		ect_parse_integer(&address,
				  &ect_new_timing_param_size->offset);
	}
				 ...
				 struct ect_pidtm_block *block)
{
	...
	block->param_name_list = kcalloc(block->num_of_parameter,
					 sizeof(char *), GFP_KERNEL);

	if (!block->param_name_list) {
		ret = -ENOMEM;
		goto err_param_name_list_allocation;
	}

	for (i = 0; i < block->num_of_parameter; ++i) {
		if (ect_parse_string(&address, &block->param_name_list[i],
				     &length)) {
			ret = -EINVAL;
...
static int ect_parse_pidtm_header(void *address, struct ect_info *info)
{
	...
	ect_pidtm_header = kzalloc(sizeof(*ect_pidtm_header), GFP_KERNEL);
	...
	ect_pidtm_header->block_list = kcalloc(ect_pidtm_header->num_of_block,
					       sizeof(struct ect_pidtm_block),
					       GFP_KERNEL);

	if (!ect_pidtm_header->block_list) {
		ret = -ENOMEM;
...
static int ect_dump_header(struct seq_file *s, void *data)
{
	...
	if (!header) {
		seq_puts(s, "[ECT] : there is no ECT Information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : ECT Information\n");
	seq_printf(s, "\t[VA] : %p\n", ect_early_vm.addr);
...
static int ect_dump_dvfs(struct seq_file *s, void *data)
{
	...
	if (!ect_dvfs_header) {
		seq_puts(s, "[ECT] : there is no dvfs information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : DVFS Information\n");
	...
	for (i = 0; i < ect_dvfs_header->num_of_domain; ++i) {
		...
		seq_puts(s, "\t\t[BOOT LEVEL IDX] : ");
		if (domain->boot_level_idx == -1)
			seq_puts(s, "NONE\n");
		else
			seq_printf(s, "%d\n", domain->boot_level_idx);

		seq_puts(s, "\t\t[RESUME LEVEL IDX] : ");
...
static int ect_dump_pll(struct seq_file *s, void *data)
{
	...
	if (!ect_pll_header) {
		seq_puts(s, "[ECT] : there is no pll information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : PLL Information\n");
	seq_printf(s, "\t[PARSER VERSION] : %d\n",
...
static int ect_dump_voltage(struct seq_file *s, void *data)
{
	...
	if (!ect_voltage_header) {
		seq_puts(s, "[ECT] : there is no asv information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : ASV Voltage Information\n");
	seq_printf(s, "\t[PARSER VERSION] : %d\n",
		...
		for (j = 0; j < domain->num_of_table; ++j) {
			...
			for (k = 0; k < domain->num_of_level; ++k) {
				...
				for (l = 0; l < domain->num_of_group; ++l) {
					if (domain->table_list[j].voltages) {
						seq_printf(s, "%u ",
							   domain->table_list[j].voltages[k *
							   domain->num_of_group + l]);
					} else if (domain->table_list[j].voltages_step) {
						seq_printf(s, "%u ",
							   domain->table_list[j].voltages_step[k *
							   domain->num_of_group + l] *
...
static int ect_dump_rcc(struct seq_file *s, void *data)
{
	...
	if (!ect_rcc_header) {
		seq_puts(s, "[ECT] : there is no rcc information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : RCC Information\n");
	seq_printf(s, "\t[PARSER VERSION] : %d\n",
	...
	for (i = 0; i < ect_rcc_header->num_of_domain; ++i) {
		...
		for (j = 0; j < domain->num_of_table; ++j) {
			...
			for (k = 0; k < domain->num_of_level; ++k) {
				seq_puts(s, "\t\t\t\t");
				for (l = 0; l < domain->num_of_group; ++l) {
					if (domain->table_list[j].rcc) {
						seq_printf(s, "%u ",
							   domain->table_list[j].rcc[k *
							   domain->num_of_group + l]);
					} else if (domain->table_list[j].rcc_compact) {
						seq_printf(s, "%u ",
							   domain->table_list[j].rcc_compact[k *
							   domain->num_of_group + l]);
					}
				}
				seq_puts(s, "\n");
...
static int ect_dump_mif_thermal(struct seq_file *s, void *data)
{
	...
	if (!ect_mif_thermal_header) {
		seq_puts(s, "[ECT] : there is no mif thermal information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : MIF Thermal Information\n");
	seq_printf(s, "\t[PARSER VERSION] : %d\n",
...
static int ect_dump_ap_thermal(struct seq_file *s, void *data)
{
	...
	if (!ect_ap_thermal_header) {
		seq_puts(s, "[ECT] : there is no ap thermal information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : AP Thermal Information\n");
	seq_printf(s, "\t[PARSER VERSION] : %d\n",
...
static int ect_dump_margin(struct seq_file *s, void *data)
{
	...
	if (!ect_margin_header) {
		seq_puts(s, "[ECT] : there is no margin information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : Margin Information\n");
	seq_printf(s, "\t[PARSER VERSION] : %d\n",
	...
	for (i = 0; i < ect_margin_header->num_of_domain; ++i) {
		...
		for (j = 0; j < domain->num_of_level; ++j) {
			...
			for (k = 0; k < domain->num_of_group; ++k) {
				...
				} else if (domain->offset_compact) {
					seq_printf(s, "%u ",
						   domain->offset_compact[j *
						   domain->num_of_group + k] * domain->volt_step);
				}
			}
			seq_puts(s, "\n");
		}
...
static int ect_dump_timing_parameter(struct seq_file *s, void *data)
{
	...
	if (!ect_timing_param_header) {
		seq_puts(s, "[ECT] : there is no timing parameter information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : Timing-Parameter Information\n");
	seq_printf(s, "\t[PARSER VERSION] : %d\n",
	...
	for (i = 0; i < ect_timing_param_header->num_of_size; ++i) {
		...
		for (j = 0; j < size->num_of_level; ++j) {
			...
			for (k = 0; k < size->num_of_timing_param; ++k) {
				seq_printf(s, "%X ",
					   size->timing_parameter[j *
					   size->num_of_timing_param + k]);
			}
			seq_puts(s, "\n");
		}
	}
...
static int ect_dump_minlock(struct seq_file *s, void *data)
{
	...
	if (!ect_minlock_header) {
		seq_puts(s, "[ECT] : there is no minlock information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : Minlock Information\n");
	seq_printf(s, "\t[PARSER VERSION] : %d\n",
...
static int ect_dump_gen_parameter(struct seq_file *s, void *data)
{
	...
	if (!ect_gen_param_header) {
		seq_puts(s, "[ECT] : there is no general parameter information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : General-Parameter Information\n");
	seq_printf(s, "\t[PARSER VERSION] : %d\n",
...
static int ect_dump_binary(struct seq_file *s, void *data)
{
	...
	if (!ect_binary_header) {
		seq_puts(s, "[ECT] : there is no binary information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : Binary Information\n");
	seq_printf(s, "\t[PARSER VERSION] : %d\n",
...
static int ect_dump_new_timing_parameter(struct seq_file *s, void *data)
{
	...
	if (!ect_new_timing_param_header) {
		seq_puts(s, "[ECT] : there is no new timing parameter information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : New Timing-Parameter Information\n");
	seq_printf(s, "\t[PARSER VERSION] : %d\n",
		   ect_new_timing_param_header->parser_version);
	seq_printf(s, "\t[VERSION] : %c%c%c%c\n",
...
static int ect_dump_pidtm(struct seq_file *s, void *data)
{
	...
	if (!ect_pidtm_header) {
		seq_puts(s, "[ECT] : there is no pidtm parameter information\n");
		return 0;
	}

	seq_puts(s, "[ECT] : PIDTM Parameter Information\n");
	seq_printf(s, "\t[PARSER VERSION] : %d\n",
	...
	for (i = 0; i < ect_pidtm_header->num_of_block; ++i) {
		...
		for (j = 0; j < block->num_of_parameter; ++j) {
			seq_printf(s, "\t\t\t[PARAMETER] : %s, %d\n",
				   block->param_name_list[j],
				   block->param_value_list[j]);
		}
	}
...
static const struct file_operations ops_all_dump = {
	.open = dump_all_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
...
static int ect_dump_init(void)
{
	...
	d = debugfs_create_file("all_dump", 0444, root, NULL,
				&ops_all_dump);
	if (!d)
		return -ENOMEM;

	d = debugfs_create_file(ect_header_info.dump_node_name,
				0444, root, &ect_header_info,
				&ect_header_info.dump_ops);
	if (!d)
		return -ENOMEM;

	for (i = 0; i < ARRAY_SIZE32(ect_list); ++i) {
		if (!ect_list[i].block_handle)
			continue;

		d = debugfs_create_file(ect_list[i].dump_node_name,
					0444, root, &ect_list[i],
					&ect_list[i].dump_ops);
		if (!d)
			return -ENOMEM;
	}

	/* NOTE(mainline): class_create() dropped the owner/module argument. */
...
struct ect_new_timing_param_size *ect_new_timing_param_get_key(void *block,
							       unsigned long long key)
{
	int i;
...
int ect_parse_binary_header(void)
{
	...
	ect_header = kzalloc(sizeof(*ect_header), GFP_KERNEL);
...
static int exynos_ect_probe(struct platform_device *pdev)
{
	...
	ect_early_vm.addr =
		exynos_ect_request_region(ect_early_vm.phys_addr,
					  ect_early_vm.size);
