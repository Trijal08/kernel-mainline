...
static void acpm_alloc_irq_info(bool usage)
{
	kfree(irq_info);
	if (usage)
		/* NOTE(mainline): nr_irqs is no longer exported; use irq_get_nr_irqs(). */
		irq_info = kcalloc(irq_get_nr_irqs(), sizeof(struct cpu_irq_info), GFP_KERNEL);
	else
		irq_info = NULL;
}
...
static void cpu_irq_info_dump(u32 retry)
{
	...
	for_each_irq_nr(i) {
		...
		for_each_possible_cpu(cpu)
			/* NOTE(mainline): kstat_irqs is now struct irqstat __percpu *. */
			irq_stat += per_cpu_ptr(desc->kstat_irqs, cpu)->cnt;
