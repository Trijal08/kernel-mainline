...
static int power_stats_probe(struct platform_device *pdev)
{
	...
	/*
	 * NOTE(mainline): devm_device_add_groups() was removed upstream. The
	 * sysfs groups are now attached via driver.dev_groups below, so the
	 * driver core adds (and removes) them around probe/remove.
	 */
