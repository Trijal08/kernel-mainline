...
/*
 * NOTE(mainline): always pull in common.h. Upstream this module may build as
 * built-in (CONFIG_GS_ACPM=y), where CONFIG_GS_ACPM_MODULE is undefined; the
 * non-module struct layout below still references struct build_info / plugin /
 * ipc_channel which are defined in common.h, so the include must be
 * unconditional to avoid incomplete-type errors.
 */
#include "common.h"
