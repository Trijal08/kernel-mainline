...
/*
 * NOTE(mainline): these structures describe the firmware-shared SRAM layout,
 * where cross-references between firmware objects are stored as u32 byte
 * offsets (not host pointers). The driver .c files unconditionally treat
 * plugins/ipc_channels/etc. as such offsets. Upstream this driver may build
 * either as a module or built-in (CONFIG_GS_ACPM=y), and the built-in case
 * leaves CONFIG_GS_ACPM_MODULE undefined, which would otherwise select the
 * host-pointer layout and mismatch the firmware ABI / the driver code. Force
 * the offset layout regardless of how the driver is linked.
 */
#ifndef CONFIG_GS_ACPM_MODULE
#define CONFIG_GS_ACPM_MODULE
#endif
