 ...
 */
/*
 * NOTE(mainline): plist_add()/plist_del() are not EXPORT_SYMBOL'd, so they are
 * unavailable to modules. Provide local reimplementations that mirror
 * lib/plist.c exactly (the CONFIG_DEBUG_PLIST plist_check_head() hooks are
 * no-ops when that option is off, so they are omitted here).
 */
static void exynos_plist_add(struct plist_node *node, struct plist_head *head)
{
	...
	last = reverse_iter = list_entry(first->prio_list.prev,
					   ...
					   s32 new_value)
{
	...
	/* NOTE(mainline): clock_set_rate trace event removed upstream; drop instrumentation. */
}
