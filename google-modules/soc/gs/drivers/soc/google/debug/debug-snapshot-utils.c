...
static void dbg_snapshot_dump_task_info(void)
{
	...
	while (curr_tsk) {
		...
		/*
		 * NOTE(mainline): task_struct.thread_group was removed; iterate
		 * the thread group via for_each_thread() and skip the group
		 * leader (already printed above as the main task).
		 */
		for_each_thread(curr_tsk, curr_thr) {
...
void dbg_snapshot_init_utils(void)
{
	...
	/* NOTE(mainline): android_vh_ipi_stop snapshot dropped; no mainline hook. */
