...
#include <linux/utsname.h>	/* __NEW_UTS_LEN */
#include <soc/google/debug-snapshot.h>
#include <soc/google/pixel-suspend-diag.h>

/*
 * NOTE(mainline): the Android-only drivers/android/debug_kinfo.h is absent
 * upstream. It describes a fixed shared-memory ABI (mirrored here by this
 * driver into a reserved-memory region for an AOSP-side consumer). Inline the
 * minimal ABI definitions so this file builds without the out-of-tree header.
 * The layout must match the AOSP debug_kinfo driver's struct kernel_info /
 * struct kernel_all_info exactly; keep field order/types in sync if updated.
 */
#define DEBUG_KINFO_MAGIC	0xCCEEDDFF

struct kernel_info {
	/* For check whether function can be called */
	u32 enabled_all;
	u32 enabled_modules;
	/* For symbols */
	u32 num_syms;
