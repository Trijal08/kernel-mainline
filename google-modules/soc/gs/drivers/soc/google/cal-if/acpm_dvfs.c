...
int exynos_acpm_dvfs_init(void)
{
	...
	if (IS_ERR(sub_node)) {
		...
		if (ret2 < 0 || acpm_dvfs.async_buffersize == 0) {
			...
			/*
			 * NOTE(mainline): devm_device_add_groups() was removed
			 * upstream; use the non-managed device_add_groups().
			 * These sysfs groups live for the lifetime of the device.
			 */
			ret2 = device_add_groups(acpm_dvfs.device, async_dvfs_groups);
			if (ret2)
