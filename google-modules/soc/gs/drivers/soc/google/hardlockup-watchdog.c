...
static void watchdog_check_hardlockup_other_cpu(void)
{
	...
	if (is_hardlockup_other_cpu(next_cpu)) {
		...
		if (hardlockup_watchdog.panic) {
			...
			/*
			 * NOTE(mainline): dump_cpu_task() is not exported to
			 * modules. Lockup detection and the subsequent panic()
			 * (which dumps all CPUs via the panic path) are retained;
			 * only the extra per-CPU task dump is dropped.
			 */
...
static void hardlockup_watchdog_enable(unsigned int cpu)
{
	...
	/* NOTE(mainline): hrtimer_init()+manual .function -> hrtimer_setup(). */
