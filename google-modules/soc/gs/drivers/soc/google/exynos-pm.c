...
/*
 * NOTE(mainline): <linux/wakeup_reason.h> and log_abnormal_wakeup_reason() are
 * an Android-only suspend wakeup-logging facility with no upstream equivalent.
 * The header include and its callers below have been dropped.
 */
...
/*
 * NOTE(mainline): MAX_SUSPEND_ABORT_LEN was provided by the Android-only
 * <linux/wakeup_reason.h>. That header has no upstream equivalent and was
 * dropped above; redefine the constant locally (matching the Android value).
 */
					     ...
					     int wakeup_stat_id)
{
	...
	/* NOTE(mainline): log_abnormal_wakeup_reason() dropped (Android-only). */
}
...
/*
 * NOTE(mainline): register_syscore_ops()/struct syscore_ops registration was
 * replaced upstream by register_syscore() taking a struct syscore wrapper.
 */
