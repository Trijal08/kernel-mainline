...
static int eh_sw_init(struct eh_device *eh_dev, int error_irq, int comp_irq)
{
	...
	/*
	 * NOTE(mainline): IRQ-affined PM QoS (pm_qos_request.type/.irq and
	 * PM_QOS_REQ_AFFINE_IRQ) is an Android-only extension with no upstream
	 * equivalent. Fall back to a global CPU latency QoS request.
	 */
