...
static const struct file_operations gsc_fops = {
	...
	/* NOTE(mainline): no_llseek was removed; non-seekable is now the default. */
	.owner		= THIS_MODULE,
};
...
static int gsa_gsc_probe(struct platform_device *pdev)
{
	...
	/* NOTE(mainline): devm_gpio_request() was removed; use the _one variant
	 * with GPIOF_IN since this GPIO is consumed as an interrupt source.
	 */
	ret = devm_gpio_request_one(dev, s->ctdl_ap_irq, GPIOF_IN, gpio_name);
	if (ret) {
		dev_err(dev, "devm_gpio_request_one failed (%d)\n", ret);
		return ret;
	}
