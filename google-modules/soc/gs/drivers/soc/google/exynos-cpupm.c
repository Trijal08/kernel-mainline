...
/*
 * NOTE(mainline): idle-state accounting via android_vh_cpu_idle_* dropped;
 * no equivalent mainline hook. pm->next_hrtimer is therefore no longer
 * refreshed from the idle loop; get_sleep_length() falls back to it as-is.
 */
...
static int exynos_cpupm_probe(struct platform_device *pdev)
{
	...
	/* NOTE(mainline): bus_type.dev_root was removed; use bus_get_dev_root(). */
	{
		struct device *cpu_root = bus_get_dev_root(&cpu_subsys);

		if (cpu_root) {
			if (sysfs_create_link(&cpu_root->kobj,
					      &pdev->dev.kobj, "cpupm"))
