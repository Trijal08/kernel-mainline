...
static void pt_handle_sysctl_register(struct pt_handle *handle)
{
	...
	/*
	 * NOTE(mainline): the recursive ctl_table "child" hierarchy and
	 * register_sysctl_table() were removed upstream. The directory path is
	 * now passed as a string to register_sysctl_sz() and the table holds
	 * only the leaf entries (plus one zero terminator).
	 */
	int leaf_cnt = handle->id_cnt;
	int entry_cnt = leaf_cnt + 1;
	char path[64];
	...
	scnprintf(path, sizeof(path), "dev/pt/%s", handle->node->name);
	handle->sysctl_header =
		register_sysctl_sz(path, sysctl_table, leaf_cnt);
	...
	const struct pt_ops *ops, void *data)
{
	...
	/*
	 * NOTE(mainline): flat ctl_table + register_sysctl_sz() with the
	 * directory path passed as a string (was a "dev"->"pt"->name child
	 * hierarchy under register_sysctl_table()).
	 */
...

	{
		...
		scnprintf(path, sizeof(path), "dev/pt/%s", driver->node->name);
		driver->sysctl_header = register_sysctl_sz(path,
...
/* NOTE(mainline): proc_handler now takes const ctl_table * and a non-user buffer. */
...
static int __init pt_init(void)
{
	...
	/*
	 * NOTE(mainline): flat ctl_table + register_sysctl_sz() with the "dev/pt"
	 * directory passed as a path string (was a child hierarchy under
	...
	pt_internal_data.sysctl_header =
		register_sysctl_sz("dev/pt", sysctl_table, 3);
