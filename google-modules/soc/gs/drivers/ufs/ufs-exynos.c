...
/*
 * NOTE(mainline): the variant pwr_change_notify callback was split in mainline.
 * Power-mode negotiation (which previously happened inside the PRE_CHANGE branch
 * of pwr_change_notify) now lives in the dedicated negotiate_pwr_mode callback,
 * which receives the device-advertised maximum capabilities and fills in the
 * agreed/final params.
 */
...
/*
 * NOTE(mainline): ufshcd_shutdown() is no longer exported by the UFS core in
 * mainline; shutdown is handled via the core's pm/remove paths. Mainline's
 * ufs-exynos provides no custom .shutdown callback, so drop it here too.
 */
