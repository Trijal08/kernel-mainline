...
#include <core/ufs_trace_types.h>
...
/*
 * NOTE(mainline): struct ufshcd_lrb no longer carries a back-pointer to the
 * SCSI command (->cmd) or the task tag (->task_tag).  In mainline the lrb is
 * stored as the SCSI command's private data via scsi_cmd_priv(), i.e.
 * scsi_cmd_priv(cmd) == (void *)(cmd + 1).  Recover the scsi_cmnd from an lrb
 * by reversing that, and derive the task tag from the block-layer request.
 *
 * The old (lrbp->cmd != NULL) test distinguished real SCSI commands from
 * internal device-management commands; replicate that by checking the request
 * UPIU transaction code.
 */
static inline struct scsi_cmnd *pixel_ufs_lrbp_to_cmd(struct ufshcd_lrb *lrbp)
{
	return (struct scsi_cmnd *)lrbp - 1;
}

static inline bool pixel_ufs_lrbp_is_scsi(struct ufshcd_lrb *lrbp)
{
	return lrbp->ucd_req_ptr &&
	       lrbp->ucd_req_ptr->header.transaction_code ==
		...
		struct ufshcd_lrb *lrbp, s64 iotime_us)
{
	...
	if (pixel_ufs_lrbp_is_scsi(lrbp)) {
		struct scsi_cmnd *cmd = pixel_ufs_lrbp_to_cmd(lrbp);
				  ...
				  enum req_type_stats *cmd_type)
{
	...
	scsi_op_code = (u8)(*pixel_ufs_lrbp_to_cmd(lrbp)->cmnd);
	switch (scsi_op_code) {
	case READ_10:
	case READ_16:
...
static void pixel_ufs_update_req_stats(struct ufs_hba *hba,
		...
		struct ufshcd_lrb *lrbp, bool is_start)
{
	...
	affected_bytes = blk_rq_bytes(scsi_cmd_to_rq(pixel_ufs_lrbp_to_cmd(lrbp)));

	/* Upload I/O amount on statistic */
	is_write = op_is_write(req_op(scsi_cmd_to_rq(pixel_ufs_lrbp_to_cmd(lrbp))));
		...
		struct ufshcd_lrb *lrbp, bool is_start)
{
	...
	if (pixel_ufs_lrbp_is_scsi(lrbp)) {
		...
		tag = lrbp->ucd_req_ptr ?
			lrbp->ucd_req_ptr->header.task_tag : 0;
...
static __maybe_unused void pixel_ufs_send_uic_command(void *data,
				struct ufs_hba *hba,
...
static __maybe_unused void pixel_ufs_send_tm_command(void *data,
				struct ufs_hba *hba,
				int tag, int str_t)
{
	enum pixel_event_type event = EVENT_UNDEF;
...
static __maybe_unused void pixel_ufs_check_int_errors(void *data,
				struct ufs_hba *hba,
				bool queue_eh_work)
{
	enum pixel_event_type event = EVENT_UNDEF;
...
static __maybe_unused void pixel_ufs_send_command(void *data,
					struct ufs_hba *hba,
					struct ufshcd_lrb *lrbp)
{
...
/*
 * NOTE(mainline): pixel UFS OCS/UPIU-result decode helpers (ufshcd_get_tr_ocs,
 * ufshcd_get_req_rsp, ufshcd_get_rsp_upiu_result) relied on the legacy
 * dword_0/1/2 layout of struct request_desc_header / utp_upiu_header and the
 * MASK_RSP_UPIU_RESULT macro, none of which survive in mainline (the headers
 * were restructured into named fields). They had no remaining callers after the
 * vendor-hook removal pass, so they are removed.
 */

static __maybe_unused void pixel_ufs_compl_command(void *data,
					struct ufs_hba *hba,
					struct ufshcd_lrb *lrbp)
{
...
static __maybe_unused void pixel_ufs_prepare_command(void *data,
			struct ufs_hba *hba,
				...
				struct device_attribute *attr, char *buf)
{
	...
	/*
	 * NOTE(mainline): ufshcd_read_desc_param() is not exported to modules
	 * in mainline UFS core, so the vendor "health descriptor" read is
	 * unavailable here. The HBA still functions; only this informational
	 * sysfs attribute is unsupported.
	 */
	return -EOPNOTSUPP;
}
			...
			struct device_attribute *attr, char *buf)
{
	...
	if (ufs->manual_gc.hagc_support) {
		/*
		 * NOTE(mainline): ufshcd_query_attr_retry() is not exported to
		 * modules upstream, so the HAGC (hardware auto-GC) status query
		 * is unavailable. Treat HAGC as unsupported; manual_gc falls
		 * back to the "bkops" reporting path below.
		 */
		ufs->manual_gc.hagc_support = false;
	}
...
static int manual_gc_enable(struct ufs_hba *hba, u32 *value)
{
	...
	/*
	 * NOTE(mainline): ufshcd_query_attr_retry() is not exported to modules
	 * upstream, so the MANUAL_GC_CONT attribute write (which enables
	 * hardware auto-GC) cannot be issued. Report not-supported; callers
	 * disable hagc_support and use the bkops fallback path.
	 */
	return -EOPNOTSUPP;
}
			...
			const char *buf, size_t count)
{
	...
	/*
	 * NOTE(mainline): the WB-buffer flush on manual GC toggled the
	 * WB_BUFF_FLUSH_{DURING_HIBERN8,EN} flags via ufshcd_query_flag(),
	 * which is not exported to modules upstream. The flush is dropped;
	 * the device flushes its write buffer on its own GC schedule, so
	 * this only affects flush timing, not correctness.
	 */

	if (!ufs->manual_gc.hagc_support) {
		/*
		 * NOTE(mainline): the software-driven manual GC fallback toggled
		 * device auto-bkops via ufshcd_bkops_ctrl(). In mainline that
		 * function (and ufshcd_{enable,disable}_auto_bkops()) is static
		 * and unexported, and its signature changed to take only @hba
		 * (status derived from hba->urgent_bkops_lvl). There is no
		 * exported replacement, so this fallback path is dropped. Manual
		 * GC therefore only operates on platforms reporting HAGC
		 * (hardware auto-GC) support; on others it now no-ops.
		 */
		err = -EOPNOTSUPP;
	}
...
void pixel_init_manual_gc(struct ufs_hba *hba)
{
	...
	/*
	 * NOTE(mainline): hrtimer_init() + .function assignment replaced by
	 * hrtimer_setup() which takes the callback as an argument.
	 */
	hrtimer_setup(&mgc->hrtimer, pixel_mgc_hrtimer_handler, CLOCK_MONOTONIC,
		      HRTIMER_MODE_REL);
				   ...
				   const char *buf, size_t count)
{
	...
	/*
	 * NOTE(mainline): toggling QUERY_FLAG_IDN_WB_BUFF_FLUSH_DURING_HIBERN8
	 * required ufshcd_query_flag(), which is not exported to modules
	 * upstream. The device-side WB flush-during-hibern8 flag is left at
	 * its default; the local set_gid state (used to tag write requests
	 * with a WB group id) is still updated.
	 */
	ufs->set_gid = i;
	return count;
}
...
/*
 * NOTE(mainline): these attributes read/write UFS device attributes via
 * ufshcd_query_attr_retry(), which is not exported to modules upstream.
 * Without an exported equivalent the show/store handlers report
 * not-supported rather than issuing the vendor query.
 */
...
/* NOTE(mainline): REG_CONTROLLER_DEV_ID/PROD_ID renamed to REG_CONTROLLER_PID/MID upstream (same offsets 0x10/0x14). */
...
static __maybe_unused void pixel_ufs_update_sysfs(void *data,
						  struct ufs_hba *hba)
{
	struct pixel_ufs *ufs = to_pixel_ufs(hba);
...
static __maybe_unused void pixel_ufs_update_sdev(void *data,
	       ...
	       const struct pixel_crypto_ops *crypto_ops)
{
	...
	/*
	 * TODO(mainline): the Pixel command/IO statistics, command logging,
	 * write-booster GID tagging, sysfs refresh and sdev tweaks were
	 * previously wired through the Android UFS vendor hooks
	 * (android_vh_ufs_prepare_command, android_vh_ufs_update_sysfs,
	 * android_vh_ufs_send_command, android_vh_ufs_compl_command,
	 * android_vh_ufs_send_uic_command, android_vh_ufs_send_tm_command,
	 * android_vh_ufs_check_int_errors, android_vh_ufs_update_sdev).  These
	 * hooks do not exist in mainline; the callbacks are retained (marked
	 * __maybe_unused) so the logic can be re-attached to ufshcd-core call
	 * sites once a mainline-friendly mechanism is available.
	 */
