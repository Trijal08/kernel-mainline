...
static int pixel_ufs_crypto_init_hw_keys_mode(struct ufs_hba *hba)
{
	...
	/*
	 * TODO(mainline): restoring all KDN keys on resume was wired via the
	 * Android UFS vendor hook android_rvh_ufs_reprogram_all_keys (backed by
	 * gsa_kdn_restore_keys()), which does not exist in mainline.  This needs
	 * to be reattached to a ufshcd resume/crypto path before wrapped-key
	 * inline encryption is fully functional across suspend/resume.
	 */
