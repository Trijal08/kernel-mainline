...
static int __init ufs_pixel_fips_init(void)
{
	...
	/*
	 * TODO(mainline): the FIPS module previously installed its own
	 * per-request PRDT crypto programming (hw- and sw-key variants) via the
	 * Android UFS vendor hook android_vh_ufs_fill_prdt, which does not exist
	 * in mainline.  Inline encryption must be reimplemented on top of
	 * ufshcd crypto ops / blk-crypto before the FIPS-mode PRDT fill can be
	 * re-enabled.  The HMAC and self-integrity self-tests above still run.
	 */
	ret = 0;
