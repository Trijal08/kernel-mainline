				      ...
				      struct page **pages, unsigned int chunk_order)
{
	...
	while (alloc_count < need_count) {
		...
		/*
		 * NOTE(mainline): Android's __cma_alloc(cma, count, align, gfp)
		 * does not exist; use mainline cma_alloc(cma, count, align,
		 * no_warn). The NORETRY/NOWARN retry heuristic below is preserved
		 * via gfp_flags, with NOWARN mapped onto cma_alloc's no_warn arg.
		 */
		page = cma_alloc(cma, 1 << alloc_order, alloc_order,
