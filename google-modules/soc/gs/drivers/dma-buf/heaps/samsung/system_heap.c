...
/*
 * NOTE(mainline): struct dma_heap_ops has no .get_pool_size in mainline, so the
 * system_heap_get_pool_size() callback (which reported the shared page-pool size
 * for the "system" heap to VTS) has been removed along with its initializer.
 */
