...
struct samsung_dma_heap {
	...
	/*
	 * NOTE(mainline): mainline dma_heap has no associated struct device and
	 * dropped dma_heap_get_dev(). Stash the registering platform device here
	 * (used for DMA cache-sync ops) as the replacement.
	 */
	struct device *dev;
...
/*
 * NOTE(mainline): the deferred-free and page-pool helpers are reimplemented
 * in-module (heaps/dma_heap_helpers.c). Their setup/teardown is hooked into
 * the module init/exit rather than a separate module_init.
 */
