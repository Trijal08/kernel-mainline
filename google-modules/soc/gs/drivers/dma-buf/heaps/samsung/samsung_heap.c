...
{
	struct device *dev = buffer->heap->dev;
						   ...
						   unsigned int flags)
{
	...
	dma_coerce_mask_and_coherent(heap->dev, DMA_BIT_MASK(36));
...
heap_put:
	/*
	 * NOTE(mainline): mainline dropped dma_heap_put() and provides no way to
	 * unregister a heap once dma_heap_add() succeeds (heaps are permanent).
	 * Any heaps already added before the failure simply remain registered.
	 */
	return ret;
}
