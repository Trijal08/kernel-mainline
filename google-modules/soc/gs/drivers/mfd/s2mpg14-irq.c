...
/* NOTE(mainline): <linux/wakeup_reason.h> is Android-only; removed along with its calls. */
...
static void s2mpg14_report_irq(struct s2mpg14_dev *s2mpg14)
{
	...
	for (i = 0; i < S2MPG14_IRQ_NR; i++) {
		if (irq_reg[s2mpg14_irqs[i].group] & s2mpg14_irqs[i].mask) {
			...
			/* NOTE(mainline): log_threaded_irq_wakeup_reason() is Android-only; removed. */
		}
	}
}
...
static irqreturn_t s2mpg14_irq_thread(int irq, void *data)
{
	...
	/* NOTE(mainline): log_abnormal_wakeup_reason() is Android-only; removed. */
