...
/*
 * NOTE(mainline): the clock_set_rate tracepoint used here is a vendor/Android
 * addition with no equivalent in mainline <trace/events/power.h>. Provide
 * no-op stubs so the (observability-only) trace call sites keep compiling.
 */
static inline bool trace_clock_set_rate_enabled(void) { return false; }
static inline void trace_clock_set_rate(const char *name, unsigned int rate,
					int cpu_id) { }
...
/*
 * Syscore operation
 * NOTE(mainline): the syscore API changed; callbacks now take a void *data
 * argument and registration is via register_syscore(struct syscore *) with the
 * ops referenced through a wrapping struct syscore.
 */
