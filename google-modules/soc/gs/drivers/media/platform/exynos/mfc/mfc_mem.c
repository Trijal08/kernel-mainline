...
/*
 * NOTE(mainline): the in-kernel dma-heap *consumer* API
 * (dma_heap_find/dma_heap_buffer_alloc/dma_heap_put) that the Android kernel
 * exported is not present in mainline 7.1 - <linux/dma-heap.h> only exposes
 * the heap *provider* side (dma_heap_add()). The static helpers below
 * reconstruct the consumer side on top of the stable
 * /dev/dma_heap/<name> character-device interface (the same shim used in
 * google-modules/gpu/mali_pixel/protected_memory_allocator.c):
 *   - dma_heap_open() stores the "struct dma_heap *" in file->private_data,
 *     so opening the node yields the heap handle while pinning the heap.
 *   - the opened "struct file *" is tracked so it can be released on put.
 *
 * Because dma_heap_put() here only receives the heap pointer (matching the
 * downstream signature used by the call sites), the open file is recorded in
 * a small static registry keyed by the heap pointer.
 *
 * Buffer allocation through this path needs the secure heaps ("mfc_fw-secure",
 * "vframe-secure") which do not exist in mainline yet; until they are ported
 * mfc_dma_heap_buffer_alloc() returns -EOPNOTSUPP so the allocation fails
 * cleanly rather than the driver mis-behaving. This path needs review once
 * the secure heaps land.
 */
#define MFC_DMA_HEAP_REG_MAX 4
static DEFINE_SPINLOCK(mfc_dma_heap_reg_lock);
static struct {
	struct dma_heap *heap;
	struct file *filp;
} mfc_dma_heap_reg[MFC_DMA_HEAP_REG_MAX];

static struct dma_heap *mfc_dma_heap_find(const char *name)
{
	struct file *filp;
	struct dma_heap *heap;
	char path[64];
	...
	heap = filp->private_data;
	if (!heap) {
		fput(filp);
		return NULL;
	}

	spin_lock(&mfc_dma_heap_reg_lock);
	for (i = 0; i < MFC_DMA_HEAP_REG_MAX; i++) {
		if (!mfc_dma_heap_reg[i].filp) {
			mfc_dma_heap_reg[i].heap = heap;
			mfc_dma_heap_reg[i].filp = filp;
			break;
		}
	}
	spin_unlock(&mfc_dma_heap_reg_lock);

	if (i == MFC_DMA_HEAP_REG_MAX) {
		/* registry full - cannot track for release; bail out */
		fput(filp);
		return NULL;
	}
...
static void mfc_dma_heap_put(struct dma_heap *heap)
{
	...
	spin_lock(&mfc_dma_heap_reg_lock);
	for (i = 0; i < MFC_DMA_HEAP_REG_MAX; i++) {
		if (mfc_dma_heap_reg[i].heap == heap &&
		    mfc_dma_heap_reg[i].filp) {
			filp = mfc_dma_heap_reg[i].filp;
			mfc_dma_heap_reg[i].heap = NULL;
			mfc_dma_heap_reg[i].filp = NULL;
			break;
		}
	}
	spin_unlock(&mfc_dma_heap_reg_lock);
...
static struct dma_buf *mfc_dma_heap_buffer_alloc(struct dma_heap *heap,
						 size_t len,
						 unsigned int fd_flags,
						 unsigned int heap_flags)
{
	/* NOTE(mainline): requires the not-yet-ported secure heaps; see above. */
		...
		struct mfc_special_buf *special_buf)
{
	...
	dma_heap = mfc_dma_heap_find(heapname);
	if (!dma_heap) {
		mfc_dev_err("Failed to get DMA heap (name: %s)\n", heapname);
		goto err_dma_heap_find;
	}

	special_buf->dma_buf = mfc_dma_heap_buffer_alloc(dma_heap,
