...
static inline void mfc_mem_buf_prepare(struct vb2_buffer *vb, int stream)
{
	...
	for (i = 0; i < vb->num_planes; i++) {
		...
		/*
		 * NOTE(mainline): partial cache flush (end_cpu_access_partial)
		 * is a downstream-only dma-buf op with no mainline analog; fall
		 * back to the full-buffer cache flush.
		 */
...
static inline void mfc_mem_buf_finish(struct vb2_buffer *vb, int stream)
{
	...
	for (i = 0; i < vb->num_planes; i++) {
		...
		/*
		 * NOTE(mainline): partial cache flush (begin_cpu_access_partial)
		 * is a downstream-only dma-buf op with no mainline analog; fall
		 * back to the full-buffer cache flush.
		 */
