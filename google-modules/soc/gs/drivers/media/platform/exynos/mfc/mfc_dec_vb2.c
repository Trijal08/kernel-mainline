...
/*
 * NOTE(mainline): mfc_dec_lock/mfc_dec_unlock backed the removed vb2_ops
 * .wait_finish/.wait_prepare callbacks. vb2 now handles dropping/retaking
 * vb2_queue.lock around waits, so these wrappers are no longer referenced
...
struct vb2_ops mfc_dec_qops = {
	...
	/*
	 * NOTE(mainline): vb2_ops .wait_prepare/.wait_finish were removed.
	 * vb2 now drops/reacquires vb2_queue.lock around blocking waits itself,
	 * so the driver only needs to set q->lock (to dev->mfc_mutex). The old
	 * mfc_dec_lock/unlock callbacks are gone.
	 */
