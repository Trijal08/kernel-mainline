...
int mfc_core_sysmmu_fault_handler(struct iommu_fault *fault, void *param)
{
	...
	/*
	 * NOTE(mainline): struct iommu_fault dropped the unrecoverable-fault
	 * union (.event); use the page-request message address (.prm.addr).
	 */
	core->logging_data->fault_addr = (unsigned int)(fault->prm.addr);

	snprintf(core->crash_info, MFC_CRASH_INFO_LEN,
		"MFC-%d SysMMU PAGE FAULT at %#010llx (AxID: %#x), fault_status: %#x\n",
		core->id, fault->prm.addr,
...
static int mfc_core_probe(struct platform_device *pdev)
{
	...
	/*
	 * NOTE(mainline): iommu_register_device_fault_handler() and the
	 * generic unrecoverable-fault reporting path were removed; mainline
	 * only keeps the recoverable I/O page-fault (iopf) model with a
	 * different handler signature. The MFC crash-logging fault handler
	 * has no mainline analog, so it is no longer registered.
	 */
...
#endif
	/* NOTE(mainline): no device fault handler to unregister (see above) */
...
#endif
	/* NOTE(mainline): no device fault handler to unregister (see probe) */
	if (timer_pending(&core->meerkat_timer))
		timer_delete(&core->meerkat_timer);
