...
/* Query a ctrl */
static __maybe_unused int mfc_dec_queryctrl(struct file *file, void *priv,
...
/* Get a ctrl */
static __maybe_unused int mfc_dec_g_ctrl(struct file *file, void *priv,
...
/* Set a ctrl */
static __maybe_unused int mfc_dec_s_ctrl(struct file *file, void *priv,
			 struct v4l2_control *ctrl)
{
	struct mfc_ctx *ctx = fh_to_mfc_ctx(file->private_data);
	struct mfc_dec *dec = ctx->dec_priv;
...
static const struct v4l2_ioctl_ops mfc_dec_ioctl_ops = {
	...
	/*
	 * NOTE(mainline): vidioc_queryctrl/g_ctrl/s_ctrl were removed from
	 * struct v4l2_ioctl_ops; the core now routes VIDIOC_QUERYCTRL/G_CTRL/
	 * S_CTRL through a registered v4l2_ctrl_handler. MFC implements its own
	 * control logic, so these single-control ioctls are dropped. The
	 * ext_ctrls path (vidioc_g_ext_ctrls) remains the supported route.
	 */
