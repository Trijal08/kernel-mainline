...
static int smfc_vb2_buf_init(struct vb2_buffer *vb)
{
	...
	for (plane = 0; plane < vb->num_planes; ++plane) {
		...
		/* NOTE(mainline): dma_buf_attachment.dma_map_attrs was removed;
		 * DMA_ATTR_PRIVILEGED can no longer be set per-attachment here.
		 */
...
static struct vb2_ops smfc_vb2_ops = {
	...
	/* NOTE(mainline): .wait_prepare/.wait_finish removed; vb2 auto-locks
	 * via q->lock.
	 */
...
/* NOTE(mainline): smfc_iommu_fault_handler() removed; the
 * iommu_register_device_fault_handler() API it was wired to no longer exists.
 */
...
static int exynos_smfc_probe(struct platform_device *pdev)
{
	 ...
	 * (mainline uses the iopf framework). The SMFC fault dump handler is
...
static void exynos_smfc_remove(struct platform_device *pdev)
{
	...
	/* NOTE(mainline): iommu_unregister_device_fault_handler() was removed
	 * along with its register counterpart.
	 */
}
