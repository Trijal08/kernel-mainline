...
static int __set_phy_cfg_0502_0000_dphy(void __iomem *regs, int option, u32 *cfg)
{
	...
	for (i = 0; i <= cfg[LANES]; i++) {
		...
		/* SD_ANA_CON2 */
		update_bits(regs + 0x0110 + (i * 0x100), 8, 2, skew_delay_sel);
		...
		/* SD_TIME_CON0 */
		update_bits(regs + 0x0130 + (i * 0x100), 0, 8, cfg[SETTLE]);
		/* SD_TIME_CON0 */
		update_bits(regs + 0x0130 + (i * 0x100), 8, 1, settle_clk_sel);
		/* SD_TIME_CON1 */
		writel(0x00000003, regs + 0x0134 + (i * 0x100));
		/* SD_DESKEW_CON0 */
		update_bits(regs + 0x0140 + (i * 0x100), 0, 1, skew_cal_en);
		/* SD_DESKEW_CON4 */
		writel(0x0000081a, regs + 0x0150 + (i * 0x100));
	}
...
static int __set_phy_cfg_0502_0001_dphy(void __iomem *regs, int option, u32 *cfg)
{
	...
	for (i = 0; i <= cfg[LANES]; i++) {
		/* SD_GNR_CON0 */
		writel(0x00000001, regs + 0x0000 + (i * 0x100));
		/* SD_GNR_CON1 */
		writel(0x00001450, regs + 0x0004 + (i * 0x100));
		/* SD_ANA_CON0 */
		writel(0x00000004, regs + 0x0008 + (i * 0x100));
		/* SD_ANA_CON1 */
		writel(0x00009000, regs + 0x000c + (i * 0x100));
		/* SD_ANA_CON2 */
		writel(0x00000005, regs + 0x0010 + (i * 0x100));
		/* SD_ANA_CON2 */
		update_bits(regs + 0x0010 + (i * 0x100), 8, 2, skew_delay_sel);
		/* RESETN_CFG_SEL */
		update_bits(regs + 0x0010 + (i * 0x100), 15, 1, 1);
		/* RXDDRCLKHS_SEL */
		update_bits(regs + 0x0010 + (i * 0x100), 7, 1, 1);
		/* SD_ANA_CON3 */
		writel(0x00000600, regs + 0x0014 + (i * 0x100));
		/* SD_TIME_CON0 */
		update_bits(regs + 0x0030 + (i * 0x100), 0, 8, cfg[SETTLE]);
		/* SD_TIME_CON0 */
		update_bits(regs + 0x0030 + (i * 0x100), 8, 1, settle_clk_sel);
		/* SD_TIME_CON1 */
		writel(0x00000003, regs + 0x0034 + (i * 0x100));
		/* SD_DESKEW_CON0 */
		update_bits(regs + 0x0040 + (i * 0x100), 0, 1, skew_cal_en);
		/* SD_DESKEW_CON4 */
		writel(0x0000081a, regs + 0x0050 + (i * 0x100));
	}
...
static int __set_phy_cfg_0502_0002_dphy(void __iomem *regs, int option, u32 *cfg)
{
	...
	if (cfg[SPEED] > 4500)
		...
		writel(0x00009000, regs + 0x000c); /* SC_ANA_CON1 */
	writel(0x00000005, regs + 0x0010); /* SC_ANA_CON2 */
	clk_div1234_mc = max(0, (int)(5 - DIV_ROUND_UP(((t_clk_miss - 1) * cfg[SPEED]) >> 2,
						       freq_s_xi_c)));
	...
	for (i = 0; i <= cfg[LANES]; i++) {
		...
		if (cfg[SPEED] > 4500)
			...
			writel(0x00009000, regs + 0x010c + (i * 0x100)); /* SD_ANA_CON1 */
		/* SD_ANA_CON2 */
		writel(0x00000005, regs + 0x0110 + (i * 0x100));
		/* SD_ANA_CON2 */
		update_bits(regs + 0x0110 + (i * 0x100), 8, 2, skew_delay_sel);
		/* SD_ANA_CON3 */
		writel(0x00000600, regs + 0x0114 + (i * 0x100));
		/* DC Combo lane has below SFR (0/1/2) */
		if ((type == 0xDC) && (i < 3))
			writel(0x00000040, regs + 0x0124 + (i * 0x100)); /* SD_ANA_CON7 */
		/* SD_TIME_CON0 */
...
static int __set_phy_cfg_0504_0000_dcphy(void __iomem *regs, int option, u32 *cfg)
{
	...
	u32 num_lane = cfg[LANES] + 1;
	u32 skew_delay_sel = 0;
	u32 settle_clk_sel = 1;
	u32 skew_cal_en = 1; /* Always enable */

	/* Disable phy for analog logic reset */
	if (mode == 0x000D) {
		writel(0x00000000, regs + 0x0000); /* SC_GNR_CON0 */

		for (i = 1; i <= num_lane; i++) {
			/* SD_GNR_CON0 */
			writel(0x00000000, regs + 0x0000 + (i * 0x100));
		}
	}
	...
	if (mode == 0x000D) {
		...
		writel(0x00000600, regs + 0x0014); /* SC_ANA_CON3 */
		writel(0x00000301, regs + 0x0030); /* SC_TIME_CON0 */
		/* SC_GNR_CON0: enable clock lane */
		writel(0x00000001, regs + 0x0000);
		...
		for (i = 1; i <= num_lane; i++) {
			...
			writel(0x00001450, regs + 0x0004 + (i * 0x100));
			/* SD_ANA_CON2 */
			writel(0x00000002, regs + 0x0010 + (i * 0x100));
			/* SD_ANA_CON2 */
			update_bits(regs + 0x0010 + (i * 0x100), 8, 2, skew_delay_sel);
			/* SD_ANA_CON3 */
			writel(0x00000600, regs + 0x0014 + (i * 0x100));
			/* SD_ANA_CON7 */
			writel(0x00000040, regs + 0x0024 + (i * 0x100));
			/* SD_TIME_CON0 */
			update_bits(regs + 0x0030 + (i * 0x100), 0, 8, cfg[SETTLE]);
			/* SD_TIME_CON0 */
			update_bits(regs + 0x0030 + (i * 0x100), 8, 1, settle_clk_sel);
			...
			/* SD_DESKEW_CON4 */
			writel(0x0000081A, regs + 0x0050 + (i * 0x100));
			/* SD_GNR_CON0: enable data lane */
			writel(0x00000001, regs + 0x0000 + (i * 0x100));
		}
	} else {
		/* TODO: Configure C-PHY mode */
	}
...
static int __set_phy_cfg_0504_0001_dcphy(void __iomem *regs, int option, u32 *cfg)
{
	int i;
	void __iomem *bias;
	u32 mode = cfg[TYPE] >> 16;
...
static int __set_phy_cfg_0504_0002_dcphy(void __iomem *regs, int option, u32 *cfg)
{
	...
	if (mode == 0x000D) {
		...
		/* clock configuration */
		writel(0x00001450, regs + 0x0204); /* SC_GNR_CON1 */
		writel(0x00000002, regs + 0x0210); /* SC_ANA_CON2 */
		writel(0x00000600, regs + 0x0214); /* SC_ANA_CON3 */
		writel(0x00000301, regs + 0x0230); /* SC_TIME_CON0 */
		/* SC_GNR_CON0: enable clock lane */
		writel(0x00000001, regs + 0x0200);
		...
		for (i = 0; i <= cfg[LANES]; i++) {
			...
			writel(0x00001450, regs + 0x0004 + (i * 0x100));
			/* SD_ANA_CON2 */
			writel(0x00008082, regs + 0x0010 + (i * 0x100));
			/* SD_ANA_CON2 */
			update_bits(regs + 0x0010 + (i * 0x100), 8, 2, skew_delay_sel);
			/* SD_ANA_CON3 */
			writel(0x00000600, regs + 0x0014 + (i * 0x100));
...
static const struct exynos_mipi_phy_cfg phy_cfg_table[] = {
	...
	},
	{
		...
		.minor = 0x0001,
		.mode = 0xD,
		.set = __set_phy_cfg_0504_0001_dcphy,
	},
	{
		/* type: DCphy, 2lane, Data lane first */
		.major = 0x0504,
		.minor = 0x0002,
		.mode = 0xD,
...
static struct exynos_mipi_phy_data mipi_phy_m0s4s4s4s4s4 = {
	.flags = MIPI_PHY_MxSx_SHARED,
	.active_count = 0,
	.slock = __SPIN_LOCK_UNLOCKED(mipi_phy_m0s4s4s4s4s4.slock),
};
