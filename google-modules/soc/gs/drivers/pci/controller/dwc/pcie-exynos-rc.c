...
static void exynos_pcie_mem_copy_epdev(struct exynos_pcie *exynos_pcie)
{
	...
	/* NOTE(mainline): struct device.dma_ops is private; use set_dma_ops(). */
	set_dma_ops(&exynos_pcie->dup_ep_dev, NULL);
...
/*
 * NOTE(mainline): dma_map_ops .map_page/.unmap_page were replaced by
 * .map_phys/.unmap_phys (commit moving the DMA op to take a phys_addr_t).
 * The callback now receives the physical address directly instead of
 * page + offset, so delegate to dma_map_phys() on the duplicated EP device.
 */
				    ...
				    unsigned long attrs)
{
	...
	dma_addr = dma_map_phys(&exynos_pcie->dup_ep_dev, phys,
				size, dir, attrs);
	if (exynos_pcie->s2mpu) {
...
static const struct dma_map_ops pcie_dma_ops = {
	...
	.map_sg = NULL,
	.unmap_sg = NULL,
	/* NOTE(mainline): .map_resource/.unmap_resource were removed from
	 * struct dma_map_ops; MMIO resource mapping now flows through
	 * .map_phys with the DMA_ATTR_MMIO attribute. No custom handling here.
	 */
...
/* NOTE(mainline): dw_pcie_ops.link_up now returns bool. */
...
static struct dw_pcie_host_ops exynos_pcie_rc_ops = {
	/* NOTE(mainline): dw_pcie_host_ops.host_init was renamed to .init
	 * and now returns int; exynos_pcie_rc_init already matches.
	 */
...
static irqreturn_t exynos_pcie_rc_irq_handler(int irq, void *arg)
{
	...
	if (val_irq2 & IRQ_MSI_RISING_ASSERT && exynos_pcie->use_msi) {
		...
		/*
		 * NOTE(mainline): dw_handle_msi_irq() is internal to the DWC
		 * host core and not exported to modules. The DWC core registers
		 * its own chained MSI demux handler (dw_chained_msi_isr ->
		 * dw_handle_msi_irq) on the MSI parent IRQ during
		 * dw_pcie_host_init(), so MSI demux is handled by the core and
		 * this vendor IRQ path must not call it directly.
		 */
...
/*
 * NOTE(mainline): no longer wired into the MSI irq_chip (see the MSI domain
 * note in exynos_pcie_rc_add_port()); kept for reference / future rewiring.
 */
static int __maybe_unused exynos_pcie_msi_set_affinity(struct irq_data *irq_data,
...
static irqreturn_t exynos_pcie_msi0_handler(int irq, void *arg)
{
	...
	/*
	 * NOTE(mainline): dw_handle_msi_irq() is internal to the DWC host core
	 * and not exported to modules. The DWC core installs its own chained
	 * MSI demux handler internally, so this separated-MSI vector handler
...
skip_sep_request_irq:
	...
	/*
	 * NOTE(mainline): struct dw_pcie_rp no longer exposes a separate
	 * .msi_domain whose host_data is a mutable msi_domain_info. In mainline
	 * 7.1 the DWC host creates a per-device MSI parent domain via
	 * msi_create_parent_irq_domain() with dw_pcie_msi_parent_ops, and the
	 * MSI irq_chip callbacks come from the generic MSI library + default
	 * chip ops rather than a single shared chip that can be patched here.
	 * The vendor overrides below (set_affinity / set_wake / mask / unmask)
	 * have no clean mainline equivalent and are dropped. This needs review
	 * if the affinity/wake behaviour is required.
	 */
...
/*
 * NOTE(mainline): the android_rvh_pci_d3_sleep vendor hook has no mainline
 * equivalent. The exynos_d3_sleep_hook probe and its registration have been
 * dropped; PCI D3 sleep timing now uses the PCI core default.
 */
...
static int exynos_pcie_rc_probe(struct platform_device *pdev)
{
	...
	/* NOTE(mainline): android_rvh_pci_d3_sleep vendor hook dropped; no mainline equivalent. */
...
static void __exit exynos_pcie_rc_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
