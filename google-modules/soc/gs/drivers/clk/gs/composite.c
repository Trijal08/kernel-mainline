 ...
 * addition with no equivalent in mainline <trace/events/power.h>. Provide a
 * no-op stub so the (observability-only) trace call sites keep compiling.
 */
...
/* NOTE(mainline): clk_ops.round_rate was replaced by .determine_rate. */
int cal_vclk_determine_rate(struct clk_hw *hw, struct clk_rate_request *req)
{
