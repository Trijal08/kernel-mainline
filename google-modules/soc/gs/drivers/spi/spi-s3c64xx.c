...
/*
 * NOTE(mainline): mainline removed the legacy spi_master* aliases (renamed to
 * spi_controller*). Provide local compat macros so the historical naming in
 * this driver (and in spi-s3c64xx-gs.h) keeps compiling unchanged.
 */
#define spi_master			spi_controller
#define spi_master_get_devdata		spi_controller_get_devdata
#define spi_master_get			spi_controller_get
#define spi_master_put			spi_controller_put
#define spi_master_suspend		spi_controller_suspend
#define spi_master_resume		spi_controller_resume
				   ...
				   struct spi_transfer *xfer)
{
	...
	/* NOTE(mainline): spi_message.is_dma_mapped was removed; caller-premapped
	 * messages are no longer supported by the SPI core, so treat as false.
	 */
	if (sci->dma_mode != DMA_MODE)
		return 0;
				      ...
				      struct spi_transfer *xfer)
{
	...
	/* NOTE(mainline): spi_message.is_dma_mapped removed; see map_one_msg. */
	if (sci->dma_mode != DMA_MODE)
		return;
...
static int s3c64xx_spi_setup(struct spi_device *spi)
{
	...
	sdd = spi_master_get_devdata(spi->controller);
