 ...
 * no equivalent mainline hook. cpu_data->idle_state is therefore no longer
 * updated from the idle loop and remains at its initialized value.
 */
...
/* If any of the above steps failed, we need to free resources. */
err_cpuhp_init:
err_client_data:
