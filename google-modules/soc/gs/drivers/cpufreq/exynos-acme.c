...
/*
 * NOTE(mainline): <linux/tensor_aio.h> and the tensor_aio_* helpers belong to
 * the out-of-tree Tensor AIO devfreq driver, which has no mainline equivalent.
 * The include and the calls into that driver are dropped; the local rate-info
 * bookkeeping (exynos-acme.h) is kept intact.
 */
...
#endif
/*
 * NOTE(mainline): clock_set_rate is an Android/vendor tracepoint that has no
 * mainline equivalent (mainline trace/events/power.h does not define it).
 * Provide local no-op stubs so the rate-change instrumentation compiles; these
 * can be wired to a real tracepoint if one is added later.
 */
static inline bool trace_clock_set_rate_enabled(void)
{
	return false;
}

static inline void trace_clock_set_rate(const char *name, unsigned int freq,
					int cpu)
{
}
				   ...
				   unsigned long capped_freq, int thermal_actor)
{
	...
	/*
	 * NOTE(mainline): the tensor_aio_cpufreq_pressure() short-circuit (Tensor
	 * AIO devfreq, out-of-tree) is dropped; always apply thermal pressure
	 * through the standard arch interface below.
	 */
	...
	if (domain->capped_freq[thermal_actor] != capped_freq) {
		...
		arch_update_hw_pressure(maskp, arch_capped_freq);
		changed = true;
	}
...
static int exynos_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	/* NOTE(mainline): tensor_aio_init_cpu_domain() (out-of-tree) dropped. */
#endif
...
static int exynos_cpufreq_verify(struct cpufreq_policy_data *new_policy)
{
	...
	/*
	 * NOTE(mainline): cpufreq_frequency_table_verify() now takes only the
	 * policy and reads new_policy->freq_table internally (set above), so the
	 * explicit freq_table argument is dropped.
	 */
...
static struct cpufreq_driver exynos_driver = {
	...
	/*
	 * NOTE(mainline): the global cpufreq_generic_attr array was removed.
	 * Its only entry (scaling_available_freqs) is now created automatically
	 * by the cpufreq core when policy->freq_table is set, so the explicit
	 * .attr assignment is dropped.
	 */
};
		     ...
		     unsigned int relation)
{
	...
	/*
	 * NOTE(mainline): cpufreq_frequency_table_target() gained explicit
	 * min/max search bounds. The DM scaler drives frequencies across the
	 * whole hardware range (not just the current policy QoS window), so pass
	 * the full cpuinfo limits to preserve the previous whole-table search.
	 */
