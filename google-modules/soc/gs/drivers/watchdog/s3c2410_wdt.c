...
static void print_wd_owner(struct s3c2410_wdt *wdt, const char *loglvl)
{
	...
	/*
	 * NOTE(mainline): dump_backtrace() is not exported to modules; the
	 * per-task backtrace dump is dropped here (lockup detection still works).
	 */
...
/*
 * NOTE(mainline): the syscore API in this tree takes a void *data callback
...
static int s3c2410wdt_probe(struct platform_device *pdev)
{
	...
	if (wdt->cluster == LITTLE_CLUSTER) {
		...
		/* NOTE(mainline): android_vh_scheduler_tick rt_load accounting dropped; no mainline hook. */
	}
