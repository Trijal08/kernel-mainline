...
/*
 * NOTE(mainline): PORT_S3C2410/2440/2412 were removed from
 * <uapi/linux/serial_core.h> (only PORT_S3C6400 survives). Define
 * driver-private values for this fork's legacy port-type switch.
 * uart_port.type is unsigned int and mainline PORT_* values max out ~123,
 * so 200+ won't collide.
 */
#ifndef PORT_S3C2410
#define PORT_S3C2410	200
#define PORT_S3C2440	201
#define PORT_S3C2412	202
#endif
...
static void exynos_serial_stop_tx(struct uart_port *port)
{
	...
	if (dma && dma->tx_chan && ourport->tx_in_progress == EXYNOS_TX_DMA) {
		...
		/* NOTE(mainline): xmit kfifo replaces circ_buf; advancing the
		 * fifo tail and tx accounting is now done by uart_xmit_advance().
		 */
...
static void exynos_serial_tx_dma_complete(void *args)
{
	...
	/* NOTE(mainline): advance xmit kfifo tail + tx accounting */
	uart_xmit_advance(port, count);
	ourport->tx_in_progress = 0;
				      ...
				      unsigned int count, unsigned int tail)
{
	...
	/*
	 * NOTE(mainline): the DMA buffer is the xmit kfifo's backing store
	 * (tport->xmit_buf); read the bytes being DMA'd straight from there at
	 * the linear @tail offset (kfifo accounting is advanced later in
	 * tx_dma_complete via uart_xmit_advance()).
	 */
	if (ourport->uart_logging && dma->tx_size)
		uart_copy_to_local_buf(0, &ourport->uart_local_buf,
				       tport->xmit_buf + tail, dma->tx_size);
...
static void exynos_serial_start_next_tx(struct exynos_uart_port *ourport)
{
	...
	/* NOTE(mainline): get linear data size + tail offset from xmit kfifo */
...
static irqreturn_t exynos_serial_tx_chars(int irq, void *id)
{
	...
	struct tty_port *tport = &port->state->port;
	unsigned long flags;
	unsigned int count, tail, dma_count = 0;
	unsigned char trace_buf[256] = {0, };
	int trace_cnt = 0;
	char buf[DATA_BYTES_PER_LINE * 3 + 1];
	...
	/* NOTE(mainline): linear data size + tail offset from xmit kfifo */
	count = kfifo_out_linear(&tport->xmit_fifo, &tail, UART_XMIT_SIZE);
	...
	/*
	 * NOTE(mainline): pull each byte from the xmit kfifo via
	 * uart_fifo_get() (which also accounts icount.tx) instead of indexing
	 * xmit->buf[xmit->tail]. @count bounds the PIO portion (the remainder
	 * is handed to DMA below); decrement it per byte sent.
	 */
	while (count > 0 &&
	       !(rd_regl(port, S3C2410_UFSTAT) & ourport->info->tx_fifofull)) {
		unsigned char ch;
...
static int exynos_serial_request_dma(struct exynos_uart_port *ourport)
{
	...
	/* TX buffer */
	/* NOTE(mainline): DMA over the xmit kfifo's backing store (xmit_buf) */
