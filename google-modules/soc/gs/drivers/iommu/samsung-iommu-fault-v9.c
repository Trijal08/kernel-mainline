...
/*
 * NOTE(mainline): enum iommu_fault_reason and struct iommu_fault_event were
 * removed along with the legacy unrecoverable-fault reporting path. Faults are
 * now reported through report_iommu_fault(); carry the raw fault data needed by
 * that call instead of a pre-built iommu_fault_event.
 */
...
static int samsung_sysmmu_fault_notifier(struct device *dev, void *data)
{
	...
	for (i = 0; i < client->sysmmu_count; i++) {
		if (drvdata == client->sysmmus[i]) {
			...
			/*
			 * NOTE(mainline): replaced iommu_report_device_fault()
			 * (now exclusively for recoverable page-request faults)
			 * with report_iommu_fault(). A return of 0 means a
			 * registered fault handler consumed the fault, which
			 * maps onto the old "-EAGAIN / retry" semantics.
			 */
			if (domain) {
...
irqreturn_t samsung_sysmmu_irq_thread(int irq, void *dev_id)
{
	...
	/*
	 * NOTE(mainline): the legacy iommu_fault_event/reason fields are gone.
	 * Pass the raw fault address/vmid to report_iommu_fault() via the
	 * notifier. An access fault is reported as a write, otherwise a read.
	 */
	fi.addr = addr;
	fi.vmid = vmid;
	fi.flags = (itype == SYSMMU_FAULT_ACCESS) ? IOMMU_FAULT_WRITE :
						    IOMMU_FAULT_READ;
