...
/*
 * NOTE(mainline): the .domain_alloc(unsigned type) op was replaced upstream by
 * .domain_alloc_paging(struct device *dev), which only ever allocates a regular
 * paging (UNMANAGED/DMA) domain; the type argument and its validation are gone.
 * pgsize_bitmap also moved out of struct iommu_ops and is now a per-domain
 * field set here.
 */
...
static struct samsung_sysmmu_domain *attach_helper(struct iommu_domain *dom, struct device *dev)
{
	...
	/*
	 * NOTE(mainline): iommu_fwspec::ops was removed; the IOMMU core only
	 * routes a device to this driver's callbacks when it matches our ops,
	 * so checking for a valid fwspec is sufficient here.
	 */
	if (!fwspec) {
		dev_err(dev, "failed to attach, IOMMU instance data is not initialized.\n");
...
/*
 * NOTE(mainline): the iommu_domain_ops::detach_dev callback was removed. The
 * equivalent behaviour is now provided through an always-attachable blocked
 * domain whose attach_dev tears down any previous translation. The IOMMU core
 * attaches this blocked domain when a device must be detached.
 */
static int samsung_sysmmu_attach_dev_blocked(struct iommu_domain *dom, struct device *dev,
					     struct iommu_domain *old)
{
	struct sysmmu_clientdata *client;
	struct sysmmu_drvdata *drvdata;
	...
	dev_info(dev, "detached (blocked domain)\n");

	return 0;
}

static int samsung_sysmmu_blocked_set_dev_pasid(struct iommu_domain *dom,
						struct device *dev, ioasid_t pasid,
						struct iommu_domain *old);

static const struct iommu_domain_ops samsung_sysmmu_blocked_ops = {
	.attach_dev = samsung_sysmmu_attach_dev_blocked,
	.set_dev_pasid = samsung_sysmmu_blocked_set_dev_pasid,
};

static struct iommu_domain samsung_sysmmu_blocked_domain = {
	.type = IOMMU_DOMAIN_BLOCKED,
	.ops = &samsung_sysmmu_blocked_ops,
};
...
static int samsung_sysmmu_map_one(struct iommu_domain *dom, unsigned long l_iova,
...
/*
 * NOTE(mainline): iommu_domain_ops::map was replaced by ::map_pages which maps
 * @pgcount pages of @pgsize. Loop over the existing single-page mapping logic.
 */
static int samsung_sysmmu_map_pages(struct iommu_domain *dom, unsigned long l_iova,
				    ...
				    int prot, gfp_t gfp, size_t *mapped)
{
	...
	while (pgcount--) {
		ret = samsung_sysmmu_map_one(dom, l_iova, paddr, pgsize, prot, gfp);
		if (ret)
			break;
		l_iova += pgsize;
		paddr += pgsize;
...
/*
 * NOTE(mainline): iommu_domain_ops::unmap (single page) was removed; unmapping
 * is now handled exclusively through ::unmap_pages below.
 */
...
/* NOTE(mainline): iommu_domain_ops::iotlb_sync_map now returns int. */
...
static struct iommu_device *samsung_sysmmu_probe_device(struct device *dev)
{
	...
	/* NOTE(mainline): iommu_fwspec::ops removed; core guarantees ops match. */
...
static void samsung_sysmmu_release_device(struct device *dev)
{
	...
	/*
	 * NOTE(mainline): iommu_fwspec_free() is no longer exported; the IOMMU
	 * core frees the fwspec when the device is released.
	 */
}
...
static int samsung_sysmmu_of_xlate(struct device *dev, const struct of_phandle_args *args)
{
...
/*
 * NOTE(mainline): the iommu_ops::remove_dev_pasid callback was removed. PASID
 * removal is now expressed as attaching the blocked domain to the PASID via
 * iommu_domain_ops::set_dev_pasid. The previously attached samsung PASID domain
 * is supplied in @old, so iommu_get_domain_for_dev_pasid() is no longer needed.
 */
static int samsung_sysmmu_blocked_set_dev_pasid(struct iommu_domain *dom,
						struct device *dev, ioasid_t pasid,
						struct iommu_domain *old)
...
static struct iommu_ops samsung_sysmmu_ops = {
	...
	/* NOTE(mainline): pgsize_bitmap moved to struct iommu_domain (set per-domain in alloc). */
	.default_domain_ops     = &(const struct iommu_domain_ops) {
		.attach_dev             = samsung_sysmmu_attach_dev,
		.set_dev_pasid		= samsung_sysmmu_set_dev_pasid,
		.map_pages              = samsung_sysmmu_map_pages,
