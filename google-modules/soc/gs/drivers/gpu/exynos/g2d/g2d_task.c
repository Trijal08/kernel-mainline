...
int g2d_create_tasks(struct g2d_device *g2d_dev)
{
	 ...
	 * modules; use the exported sched_set_fifo() helper which assigns a
	 * mid-range SCHED_FIFO priority (MAX_RT_PRIO/2) to the kthread.
	 */
	sched_set_fifo(g2d_dev->completion_workq->task);
	...
	sched_set_fifo(g2d_dev->schedule_workq->task);
