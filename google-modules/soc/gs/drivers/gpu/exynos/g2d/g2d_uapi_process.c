			   ...
			   enum dma_data_direction dir)
{
	...
	/* NOTE(mainline): get_vaddr_frames() gained a 'write' arg; pages must be
	 * writable when the device writes into the buffer (DMA_FROM_DEVICE).
	 */
	ret = get_vaddr_frames(begin << PAGE_SHIFT, nr_pages,
			       dir == DMA_FROM_DEVICE || dir == DMA_BIDIRECTIONAL, vec);
	if (ret < 0)
		goto err_get_frames;
	if (ret != (int)nr_pages) {
		ret = -EFAULT;
