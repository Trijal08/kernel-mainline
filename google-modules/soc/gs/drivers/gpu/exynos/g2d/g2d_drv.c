...
/*
 * NOTE(mainline): iommu_register_device_fault_handler()/iommu_fault were removed
 * in mainline (the per-device IOMMU fault callback was replaced by the iopf/SVA
 * fault queue model). This handler was debug-only (dump on MMU fault) and has no
 * drop-in mainline equivalent, so it has been dropped along with its registration.
 */
...
static int g2d_probe(struct platform_device *pdev)
{
	...
	/* NOTE(mainline): IOMMU device fault handler registration removed (see above). */
...
err_dt:
	/* NOTE(mainline): IOMMU device fault handler unregistration removed (see above). */
