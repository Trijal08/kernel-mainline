...
void g2d_fence_timeout_handler(struct timer_list *arg)
{
	...
	for (i = 0; i < task->num_source; i++) {
		...
		if (fence) {
			strscpy(name, fence->ops->get_driver_name(fence), sizeof(name));
...
/* NOTE(mainline): dma_fence_ops::fence_value_str was removed; the helper that
 * formatted the seqno into a string is no longer wired up by the fence core.
 */
