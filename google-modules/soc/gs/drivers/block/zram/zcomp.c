...
int zcomp_decompress(struct zcomp *comp, u32 index, struct page *page)
{
	...
	if (src_len == PAGE_SIZE) {
		/*
		 * NOTE(mainline): zs_map_object()/zs_unmap_object() were replaced
		 * by zs_obj_read_begin()/zs_obj_read_end(). For a full-page object
		 * the data never spans zspages, so no local copy buffer is needed.
		 */
	...
	/*
	 * NOTE(mainline): a sub-page compressed object may span two zspages, so
	 * zs_obj_read_begin() needs a contiguous scratch buffer to linearise
	 * into. This Google zcomp design has no per-stream scratch buffer, so
	 * allocate a temporary one for the decompress.
	 */
		      ...
		      struct zcomp_cookie *cookie)
{
	...
	/*
	 * NOTE(mainline): __GFP_CMA is an Android-only GFP flag with no mainline
	 * equivalent; dropped. zs_malloc() also gained a NUMA node id argument.
	 */
	...
	/*
	 * NOTE(mainline): zs_map_object()/memcpy/zs_unmap_object() were replaced
	 * by zs_obj_write(), which copies from a contiguous source buffer into
	 * the (possibly page-spanning) object.
	 */
