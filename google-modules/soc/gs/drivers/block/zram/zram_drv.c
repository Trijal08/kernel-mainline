...
static void init_zram_stat(struct zram *zram)
{
	int cpu;
		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	down_read(&zram->init_lock);
	if (init_done(zram)) {
		atomic_long_set(&zram->max_used_pages,
		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (!sysfs_streq(buf, "all")) {
		...
		if (IS_ENABLED(CONFIG_ZRAM_GS_MEMORY_TRACKING) && !kstrtoull(buf, 0, &age_sec))
...
/*
 * NOTE(mainline): page_endio() was removed from mainline. Reimplement its
 * old semantics locally: a read completion sets up-to-date state and unlocks
 * the page; a write completion records any error in the mapping and ends
 * writeback. Defined unconditionally since zram_page_write_endio() (outside
 * the GS_WRITEBACK block) also uses it.
 */
static void zram_page_endio(struct page *page, bool is_write, int err)
{
	if (!is_write) {
		folio_end_read(page_folio(page), err == 0);
	} else {
		if (err) {
...
static void zram_page_end_io(struct bio *bio)
{
	...
	zram_page_endio(page, op_is_write(bio_op(bio)),
...
/*
 * NOTE(mainline): the block-layer ->rw_page interface was removed entirely in
 * mainline (along with block_device_operations::rw_page). The zram_rw_page()
 * callback and its registration are therefore dropped; all I/O now goes through
 * the bio path (zram_submit_bio).
 */
...
/* NOTE(mainline): ->open now takes (struct gendisk *, blk_mode_t). */
...
{
	/*
	 * NOTE(mainline): queue configuration moved from the deprecated
	 * blk_queue_*() helpers / QUEUE_FLAG_* into struct queue_limits, which is
	 * now passed to blk_alloc_disk().  QUEUE_FLAG_NONROT is the default and
	 * QUEUE_FLAG_ADD_RANDOM was removed; QUEUE_FLAG_STABLE_WRITES became
	...
	/*
	 * NOTE(mainline): all queue configuration above is now expressed via the
	 * queue_limits passed to blk_alloc_disk(); see the 'lim' initializer.
	 */
