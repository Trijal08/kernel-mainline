...
#include <linux/gpio/driver.h>	/* gpio_chip, GPIO_LINE_DIRECTION_OUT */
...
{
	/* NOTE(mainline): GPIOF_DIR_OUT removed; use GPIO_LINE_DIRECTION_OUT. */
...
/* NOTE(mainline): gpio_chip.set callback now returns int. */
...
static int ext_bst_en_gpio_init(struct max77759_plat *chip)
{
	...
	/* NOTE(mainline): gpio_chip.of_node replaced by .fwnode. */
	chip->gpio.fwnode = of_fwnode_handle(of_find_node_by_name(chip->dev->of_node,
								  chip->gpio.label));
...
static int process_rx(struct max77759_plat *chip, u16 status)
{
	...
	/* NOTE(mainline): tcpm_pd_receive() gained an rx_sop_type arg; only
	 * SOP frames reach here (verified above).
	 */
...
/*
 * NOTE(mainline): typec vendor hook android_vh_typec_store_partner_src_caps
 * dropped; no mainline equivalent. Partner source caps are no longer captured
 * from TCPM via this hook, so tcpm_get_partner_src_caps() will report no data.
 */
...
/*
 * NOTE(mainline): the typec vendor hooks (android_vh_typec_tcpm_get_timer,
 * android_vh_typec_tcpm_log, android_vh_typec_tcpm_modify_src_caps,
 * android_vh_typec_store_partner_src_caps) have no mainline equivalent. Their
 * probe callbacks and the registration calls have been dropped. As a result:
 *   - custom TCPM timer overrides are not applied;
 *   - TCPM logs are not redirected into the local logbuffer;
 *   - source PDO current limiting (limit_src_cap sysfs) is not enforced;
 *   - partner source caps are not captured.
 * This is a no-op stub kept so the call site in probe stays unchanged.
 */
 ...
 * return void (one-shot unless re-armed).
 */
...
/* NOTE(mainline): alarm callbacks now return void. */
