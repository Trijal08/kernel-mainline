...
/* NOTE(mainline): host/xhci-rcar.h was an out-of-tree quirk header and is not
 * referenced anywhere in this driver; dropped.
 */
...
/* NOTE(mainline): <trace/hooks/usb.h> Android vendor hooks have no mainline
 * equivalent; include and hook calls dropped.
 */
...
static int __maybe_unused xhci_exynos_suspend(struct device *dev)
{
	...
	/* NOTE(mainline): trace_android_rvh_usb_dev_suspend() vendor hook dropped;
	 * default behavior is to never bypass suspend.
	 */
...
static int __maybe_unused xhci_exynos_resume(struct device *dev)
{
	...
	/* NOTE(mainline): trace_android_vh_usb_dev_resume() vendor hook dropped;
	 * default behavior is to never bypass resume.
	 */
	...
	/* NOTE(mainline): xhci_resume() gained an is_auto_resume argument. */
	ret = xhci_resume(xhci, 0, false);
	if (ret) {
