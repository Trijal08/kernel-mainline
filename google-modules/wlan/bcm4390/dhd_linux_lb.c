...
dhd_napi_schedule(void *info)
{
	...
	if (napi_schedule_prep(&dhd->rx_napi_struct)) {
		...
		/*
		 * NOTE(mainline): raise_softirq() is not exported to modules in
		 * mainline 7.1. It was used here only as an extra nudge to wake
		 * ksoftirqd after queueing the NAPI poll. __napi_schedule()
		 * above already calls __raise_softirq_irqoff(NET_RX_SOFTIRQ)
		 * internally (raising NET_RX_SOFTIRQ and waking ksoftirqd when
		 * needed), so dropping this redundant call leaves the RX NAPI
		 * scheduling path functionally intact.
		 */
