...
dhd_update_fw_path(dhd_pub_t *dhdpub, const char *fw_path)
{
	...
	if (fw_path != NULL && fw_path[0] != '\0') {
		...
		ASSERT(dhdinfo != NULL);
		(void)strscpy(dhdinfo->fw_path, fw_path, sizeof(dhdinfo->fw_path));
#ifdef SHOW_LOGTRACE
	...
	if (ret < 0)
		...
		if (ver_temp_buf == NULL) {
			...
			if (ver_temp_buf == NULL) {
				...
				strscpy(clm_version, ver_temp_buf,
...
dhd_print_kirqstats(dhd_pub_t *dhd, unsigned int irq_num)
{
	...
	for_each_online_cpu(i)
		/* NOTE(mainline): desc->kstat_irqs is now struct irqstat __percpu *;
		 * use irq_desc_kstat_cpu() (linux/irqdesc.h) which reads the per-cpu
		 * .cnt and is NULL-safe.
		 */
