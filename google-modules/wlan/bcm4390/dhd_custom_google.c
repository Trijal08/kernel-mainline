...
/*
 * NOTE(mainline): <linux/pcie_google_if.h> is the LGA/newer-SoC PCIe glue header
 * and is not present in this tree (the matching controller driver is unported).
 * Guard the include with __has_include so an Exynos-GS build that has not
 * selected CONFIG_PCI_EXYNOS_GS (e.g. due to a defconfig gap) does not fatally
 * fail on a missing header. When the header is absent the !PCI_EXYNOS_GS branch
 * below still references google_pcie_* symbols that this tree does not provide;
 * such a build must enable CONFIG_PCI_EXYNOS_GS for zumapro hardware.
 */
#elif __has_include(<linux/pcie_google_if.h>)
...
dhd_get_platform_naming_for_nvram_clmblob_file(download_type_t component, char *file_name)
{
	...
	for (i = 0; i < MAX_FILE_COUNT; i++) {
		...
		if (error == BCME_OK) {
			...
			strscpy(file_name, tmp_fname, MAX_FILE_LEN);
			break;
		}
	}
	return error;
