...
static s32 wl_cfg80211_get_tx_power(struct wiphy *wiphy,
	struct wireless_dev *wdev, int radio_idx, unsigned int link_id, s32 *dbm);
#else
...
wl_cfg80211_config_default_mgmt_key(struct wiphy *wiphy,
	struct wireless_dev *wdev, int link_id, u8 key_idx)
#else
static s32
wl_cfg80211_config_default_mgmt_key(struct wiphy *wiphy,
	struct net_device *dev, u8 key_idx)
...
/*
 * NOTE(mainline): cfg80211_ops .change_beacon now takes a struct
 * cfg80211_ap_update *; the underlying driver routine (in wl_cfgvif.c) still
 * operates on cfg80211_beacon_data. Adapt here without touching wl_cfgvif.c.
 */
static int
wl_cfg80211_update_ap_wrapper(struct wiphy *wiphy, struct net_device *dev,
	struct cfg80211_ap_update *info)
{
	return wl_cfg80211_change_beacon(wiphy, dev, &info->beacon);
}

#ifdef WL_CFG80211_MONITOR
/*
 * NOTE(mainline): cfg80211_ops .set_monitor_channel gained a struct
 * net_device * argument. The driver routine (in wl_cfgvif.c) does not need it
 * (it resolves the monitor netdev internally), so accept and ignore it here.
 */
static int
wl_cfg80211_set_monitor_channel_wrapper(struct wiphy *wiphy,
	struct net_device *dev, struct cfg80211_chan_def *chandef)
{
	BCM_REFERENCE(dev);
	return wl_cfg80211_set_monitor_channel(wiphy, chandef);
}
#endif /* WL_CFG80211_MONITOR */
...
#else
	.change_beacon = wl_cfg80211_update_ap_wrapper,
...
#ifdef WL_CFG80211_MONITOR
	.set_monitor_channel = wl_cfg80211_set_monitor_channel_wrapper,
#endif /* WL_CFG80211_MONITOR */
#ifdef GTK_OFFLOAD_SUPPORT
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 1, 0))
	...
	bool notify, bool user_enforced, int revinfo)
{
	...
	/* NOTE(mainline): strscpy() returns -E2BIG on truncation (strlcpy returned
	 * the source length), so detect truncation via a negative return.
	 */
	if (strscpy(cfg->country, country_code, WL_CCODE_LEN) < 0) {
...
int wl_get_bss_info(struct bcm_cfg80211 *cfg, struct net_device *dev, const u8 *mac)
{
	...
	strscpy(cfg->bss_info, "x x x x x x x x x x x x x x x x x", sizeof(cfg->bss_info));
...
{
	/*
	 * NOTE(mainline): struct wireless_dev::mtx was removed upstream;
	 * cfg80211 now serialises per-wdev operations under the wiphy mutex.
	 * Review for re-entrancy if these helpers are called from a context
	 * that already holds the wiphy lock.
	 */
