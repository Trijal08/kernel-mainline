...
/*
 * NOTE(mainline): the AOC (always-on-compute) module is not wired into this
 * build, so the TBN service API it provides has no in-tree provider. Define
 * weak fallbacks so the touch bus negotiator links standalone and degrades to
 * non-AOC behaviour (AOC never "ready"). If the aoc module is later built and
 * exports strong symbols, those override these. Remove once aoc is enabled.
 */
__weak bool aoc_tbn_service_ready(void) { return false; }
__weak ssize_t aoc_tbn_service_read(void *cmd, size_t size) { return -ENODEV; }
__weak ssize_t aoc_tbn_service_write(void *cmd, size_t size) { return -ENODEV; }
#endif
...
static int tbn_probe(struct platform_device *pdev)
{
	...
	if (tbn->mode == TBN_MODE_GPIO) {
		...
		if (gpio_is_valid(tbn->aoc2ap_gpio)) {
			/* NOTE(mainline): GPIOF_DIR_IN removed; use GPIOF_IN. */
			err = devm_gpio_request_one(tbn->dev, tbn->aoc2ap_gpio,
				GPIOF_IN, "tbn,aoc2ap_gpio");
			if (err) {
		...
		/* NOTE(mainline): was SCHED_FIFO prio 10; mapped to the exported
		 * sched_set_fifo() (mid-range RT prio). The helper returns void,
		 * so the former error path is no longer reachable.
		 */
