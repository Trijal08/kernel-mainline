...
/* V4L2 structures */
/*
 * NOTE(mainline): vb2_ops::wait_prepare/wait_finish were removed; vb2 now drops
 * and re-acquires q->lock automatically (q->lock is set in heatmap_probe()).
 */
...
static const struct vb2_queue heatmap_queue = {
	...
	/* NOTE(mainline): vb2_queue::min_buffers_needed renamed to min_queued_buffers. */
		...
		struct v4l2_capability *cap)
{
	...
	strscpy(cap->driver, v4l2->parent_dev->driver->name,
		sizeof(cap->driver));
	if (v4l2->input_dev != NULL && v4l2->input_dev->name != NULL) {
		strscpy(cap->card, v4l2->input_dev->name, sizeof(cap->card));
	} else {
		strscpy(cap->card, KBUILD_MODNAME, sizeof(cap->card));
	}

	strscpy(cap->bus_info, dev_name(v4l2->parent_dev),
		sizeof(cap->bus_info));
		...
		struct v4l2_input *video_input)
{
	...
	strscpy(video_input->name, "strength", sizeof(video_input->name));
	return 0;
}
...
int heatmap_probe(struct v4l2_heatmap *v4l2)
{
	...
	/* NOTE(mainline): V4L2_DEVICE_NAME_SIZE was removed; size the copy from the field. */
	strscpy(v4l2->device.name, dev_name(v4l2->parent_dev),
