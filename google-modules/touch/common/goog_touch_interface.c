...
/* NOTE(mainline): drm_bridge_funcs::attach gained a struct drm_encoder *encoder arg. */
static int panel_bridge_attach(struct drm_bridge *bridge, struct drm_encoder *encoder,
...
{
	/*
	 * NOTE(mainline): struct pm_qos_request no longer carries .type/.irq and
	 * PM_QOS_REQ_AFFINE_IRQ does not exist. IRQ-affined cpu-latency QoS is an
	 * Android extension with no mainline equivalent; fall back to a global
	 * cpu-latency QoS request (still functional, just not IRQ-affined).
	 */
	(void)irq;
