...
/*
 * NOTE(mainline): POWER_SUPPLY_CHARGE_TYPE_TAPER_EXT is a Google downstream
 * extension to the upstream power_supply charge-type enum (it was added right
 * after POWER_SUPPLY_CHARGE_TYPE_BYPASS in the Android tree). Mainline does not
 * define it, so provide the extension value here for all bms consumers.
 */
#ifndef POWER_SUPPLY_CHARGE_TYPE_TAPER_EXT
#define POWER_SUPPLY_CHARGE_TYPE_TAPER_EXT (POWER_SUPPLY_CHARGE_TYPE_BYPASS + 1)
#endif
...
/*
 * NOTE(mainline): no_llseek was removed; not setting .llseek now yields the
 * same (non-seekable) behaviour by default.
 */
...
/* NOTE(mainline): no_llseek removed; default .llseek is non-seekable. */
...
/*
 * NOTE(mainline): tcpm_update_sink_capabilities() is not exported by mainline's
 * TCPM (it was a downstream-only addition, originally reverted upstream). Keep a
 * no-op stub so sink-PDO renegotiation requests compile; dynamic sink-cap update
 * is a no-op until an upstream equivalent lands.
 * TODO(b/215766959): wire to an upstream API when available.
 */
struct tcpm_port;
