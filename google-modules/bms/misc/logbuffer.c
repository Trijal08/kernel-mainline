...
/*
 * NOTE(mainline): this tree's syscore API takes a void *data callback arg and
 * registers via register_syscore(struct syscore *) with the ops referenced
 * through a wrapping struct syscore.
 */
static const struct syscore_ops logbuffer_syscore_ops = {
	.suspend        = logbuffer_suspend,
};

static struct syscore logbuffer_syscore = {
	.ops = &logbuffer_syscore_ops,
};
