...
/* NOTE(mainline): alarm callbacks now return void (no alarmtimer_restart). */
static void
...
static struct power_supply *get_tcpm_psy(struct chg_drv *chg_drv)
{
	...
	/*
	 * NOTE(mainline): power_supply_get_by_phandle_array() is gone. Resolve
	 * the single phandle reference under "google,tcpm-power-supply" via
	 * power_supply_get_by_reference() and validate it is a tcpm supply.
	 */
	psy = power_supply_get_by_reference(of_fwnode_handle(chg_drv->device->of_node),
					    "google,tcpm-power-supply");
	if ((IS_ERR(psy) || !psy) && !chg_drv->usb_skip_probe) {
		int ret = IS_ERR(psy) ? PTR_ERR(psy) : -EAGAIN;
		...
		return ERR_PTR(ret);
	} else if (!IS_ERR(psy) && psy) {
		if (psy->desc && psy->desc->name &&
		    !strncmp(psy->desc->name, "tcpm", strlen("tcpm")))
			tcpm_psy = psy;
		else
