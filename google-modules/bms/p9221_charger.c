...
static void p9221_set_offline(struct p9221_charger_data *charger)
{
	...
	__pm_relax(charger->icl_stable_ws);
}
...
static void p9221_icl_stable_work(struct work_struct *work)
{
	struct p9221_charger_data *charger = container_of(work,
			struct p9221_charger_data, icl_stable_work.work);
...
static void p9221_soc_work(struct work_struct *work)
{
	...
	if (!charger->batt_psy) {
		...
		psy = power_supply_get_by_reference(
				of_fwnode_handle(charger->dev->of_node),
				"idt,fuel-gauge");
		...
		dev_info(charger->dev, "Reading CSP from %s\n",
			 psy->desc && psy->desc->name ? psy->desc->name : "<>");
		charger->batt_psy = psy;
	}
...
static int p9221_charger_probe(struct i2c_client *client)
{
	...
	INIT_DELAYED_WORK(&charger->icl_stable_work, p9221_icl_stable_work);
	...
	charger->icl_stable_ws = wakeup_source_register(NULL, "p9221_icl_stable");
	...
	if (charger->pdata->chip_id == P9412_CHIP_ID ||
		...
		gpio_node = of_find_node_by_name(client->dev.of_node,
						charger->gpio.label);
		charger->gpio.fwnode = of_fwnode_handle(gpio_node);
