...
static const struct power_supply_desc gcpm_pps_psy_desc = {
	...
	/* POWER_SUPPLY_PROP_USB_TYPE requires a bitmask of these */
...
static int google_cpm_probe(struct platform_device *pdev)
{
	...
	/* gcpm_pps_psy_cfg.fwnode is used to find out the snk_pdos */
	gcpm_pps_psy_cfg.drv_data = gcpm;
	gcpm_pps_psy_cfg.fwnode = of_fwnode_handle(pdev->dev.of_node);
