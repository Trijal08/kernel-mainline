...
/*
 * NOTE(mainline): power_supply_get_by_phandle_array() is gone. Resolve the
 * single phandle reference under "google,tcpm-power-supply" via
 * power_supply_get_by_reference() and validate it is a tcpm supply. @size is
 * kept for source compatibility with callers.
 */
struct power_supply *pps_get_tcpm_psy(struct device_node *node, size_t size)
{
	...
	psy = power_supply_get_by_reference(of_fwnode_handle(node), propname);
	if (IS_ERR(psy) || !psy)
		return ERR_PTR(-EAGAIN);

	name = psy->desc ? psy->desc->name : NULL;
	if (!name || strncmp(name, "tcpm", 4)) {
