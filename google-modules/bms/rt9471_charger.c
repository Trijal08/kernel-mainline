...
static int rt9471_parse_dt(struct rt9471_chip *chip)
{
	...
	char *ceb_name = NULL;
	/* NOTE(mainline): GPIOF_DIR_OUT request flag removed; output default
	 * is GPIOF_OUT_INIT_LOW (overridden to _HIGH below when configured).
	 */
...
static int rt9471_register_psy(struct rt9471_chip *chip)
{
	...
	chip->psy_cfg.fwnode = of_fwnode_handle(chip->dev->of_node);
...
static int rt9471_probe(struct i2c_client *client)
{
	...
	if (chip->dev_id == RT9470_DEVID) {
		...
		chip->gpio.fwnode = of_fwnode_handle(
				of_find_node_by_name(client->dev.of_node,
						     chip->gpio.label));
		if (!chip->gpio.fwnode)
