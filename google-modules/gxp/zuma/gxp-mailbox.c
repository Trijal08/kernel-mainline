						     ...
						     void *data, int core_id)
{
	...
	if (!IS_ERR(task)) {
		...
		/* NOTE(mainline): was SCHED_FIFO prio GXP_RT_THREAD_PRIORITY (2);
		 * mapped to the exported sched_set_fifo_low() (FIFO prio 1). The
		 * helper returns void, so the prior warn/dbg paths are dropped.
		 */
		sched_set_fifo_low(task);
		dev_dbg(dev, "response task %d set to RT prio", core_id);
	}
...
static int enable_mailbox(struct gxp_mailbox *mailbox)
{
	...
	/* Skip registering the client->AP interrupt for Tx simplex communication. */
	if (mailbox->mode != GXP_MBOX_TX_SIMPLEX)
