...
/*
 * NOTE(mainline): the per-device IOMMU fault-handler API
 * (iommu_register_device_fault_handler/iommu_unregister_device_fault_handler)
 * and the unrecoverable-fault report path (IOMMU_FAULT_DMA_UNRECOV and
 * struct iommu_fault::event) were removed upstream; faults are now delivered
 * via the iopf group infrastructure, which has no drop-in equivalent for this
 * driver's informational logging. The handler and its (un)registration have
 * been dropped. Needs review if fault reporting must be restored via iopf.
 */
...
int gxp_dma_init(struct gxp_dev *gxp)
{
	...
	/* NOTE(mainline): IOMMU fault-handler registration dropped (see top of file). */
...
{
	/* NOTE(mainline): IOMMU fault-handler unregistration dropped (see top of file). */
}
