...
int gcip_domain_pool_init(struct device *dev, struct gcip_domain_pool *pool, unsigned int size)
{
	...
	for (i = 0; i < size; i++) {
		/* NOTE(mainline): iommu_domain_alloc() removed; use iommu_paging_domain_alloc()
		 * which returns an ERR_PTR (never NULL) on failure.
		 */
...
struct iommu_domain *gcip_domain_pool_alloc(struct gcip_domain_pool *pool)
{
	...
	if (!pool->size) {
		...
		if (!ddomain)
			return NULL;
		/* NOTE(mainline): iommu_paging_domain_alloc() returns ERR_PTR on failure. */
		ddomain->domain = iommu_paging_domain_alloc(pool->dev);
		if (IS_ERR(ddomain->domain)) {
			vfree(ddomain);
			return NULL;
		}
