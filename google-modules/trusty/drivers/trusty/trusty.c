...
static int trusty_probe(struct platform_device *pdev)
{
	...
	for_each_possible_cpu(cpu) {
		...
		/*
		 * NOTE(mainline): kthread_set_per_cpu() is not exported to
		 * modules in mainline 7.1. It only sets kthread->cpu (already
		 * set by kthread_create_on_cpu() above) and the
		 * KTHREAD_IS_PER_CPU flag used by the smpboot per-cpu hotplug
		 * machinery. The thread is still bound to @cpu via the
		 * kthread_bind() that kthread_create_on_cpu() performs
		 * internally, so the per-cpu affinity is preserved; only the
		 * hotplug-helper flag is dropped.
		 */
		kthread_park(tw->nop_thread);
	}
