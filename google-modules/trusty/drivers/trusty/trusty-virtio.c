...
/*
 * NOTE(mainline): virtio_config_ops::find_vqs now passes a single
 * struct virtqueue_info[] (name/callback/ctx) instead of separate
 * callbacks[]/names[]/ctxs[] arrays.
 */
				  ...
				  struct irq_affinity *desc)
{
	...
	for (i = 0; i < nvqs; i++) {
		...
		vqs[i] = _find_vq(vdev, i, vqi->callback, vqi->name, vqi->ctx);
		if (IS_ERR(vqs[i])) {
			ret = PTR_ERR(vqs[i]);
			_del_vqs(vdev);
			return ret;
...
/*
 * NOTE(mainline): dma_map_ops::map_page was replaced by ::map_phys, which is
 * handed a phys_addr_t (page offset already folded in) instead of page+offset.
 */
