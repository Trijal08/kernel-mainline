...
static struct power_supply *google_get_power_supply(struct bcl_device *bcl_dev, const char *str)
{
	...
	/*
	 * NOTE(mainline): power_supply_get_by_phandle_array() was removed; use
	 * power_supply_get_by_reference() which resolves a single power supply
	 * referenced by the named device-tree property (the driver only ever
	 * consumed the first entry of the returned array).
	 */
	batt_psy = power_supply_get_by_reference(dev_fwnode(bcl_dev->device), str);
	if (IS_ERR(batt_psy))
		return NULL;
...
{
	return google_bcl_set_soc(thermal_zone_device_priv(tz), low, high);
}
...
{
	return google_bcl_read_soc(thermal_zone_device_priv(tz), val);
}
...
static void setup_batoilo_ifpmic_qbat_open(struct bcl_device *bcl_dev, bool enable)
{
	if (!IS_ENABLED(CONFIG_SOC_ZUMAPRO))
		return;
...
void google_bcl_enable_timer(struct bcl_device *bcl_dev)
{
	...
	/*
	 * NOTE(mainline): hrtimer_init() + assigning .function were replaced by
	 * hrtimer_setup(), which initialises the timer and its callback together.
	 */
	hrtimer_setup(&(bcl_dev->hr_timer), bcl_hrtimer_irq_handler,
...
static int google_bcl_probe(struct platform_device *pdev)
{
	...
	/* NOTE(mainline): power_supply_get_by_phandle() -> power_supply_get_by_reference(). */
	batt_psy = power_supply_get_by_reference(dev_fwnode(&pdev->dev), PSY_NAME);
