...
{
	/*
	 * NOTE(mainline): this published battery capacity/cycle-count/voltage/
	 * current as the Android-only "clock_set_rate" systrace counters (clock
	 * event class removed from mainline <trace/events/power.h>). No mainline
	 * equivalent; the markers (and the power_supply reads that only fed them)
	 * are dropped.
	 */
}
