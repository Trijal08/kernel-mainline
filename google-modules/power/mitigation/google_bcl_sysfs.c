...
/*
 * NOTE(mainline): struct thermal_zone_device is opaque and its trip table is
 * owned by the thermal core (these are OF-registered zones). The trip
 * temperature of trip 0 used to be poked directly via tz->trips[0].temperature.
 * The mainline-correct way is thermal_zone_set_trip_temp() with a real trip
 * pointer, which is reached by iterating the zone's trips under its lock via
 * thermal_zone_for_each_trip(). This helper sets trip 0's temperature and lets
 * the callback's return value stop the iteration after the first trip; it also
 * triggers the trip-change notification / re-evaluation internally.
 */
struct bcl_trip_temp_setter {
	struct thermal_zone_device *tz;
	int temp;
};

static int bcl_set_first_trip_temp_cb(struct thermal_trip *trip, void *data)
{
	struct bcl_trip_temp_setter *args = data;

	thermal_zone_set_trip_temp(args->tz, trip, args->temp);

	/* Return non-zero to stop after the first (index 0) trip. */
	return 1;
}

static void bcl_set_trip0_temperature(struct thermal_zone_device *tz, int temp)
{
	struct bcl_trip_temp_setter args = { .tz = tz, .temp = temp };

	thermal_zone_for_each_trip(tz, bcl_set_first_trip_temp_cb, &args);
}
			       ...
			       struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (bcl_dev->zone[UVLO1]->tz) {
		bcl_set_trip0_temperature(bcl_dev->zone[UVLO1]->tz,
			       ...
			       struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (bcl_dev->zone[UVLO2]->tz) {
		bcl_set_trip0_temperature(bcl_dev->zone[UVLO2]->tz,
					  VD_BATTERY_VOLTAGE - value);
				  ...
				  struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (bcl_dev->zone[BATOILO1]->tz) {
		bcl_set_trip0_temperature(bcl_dev->zone[BATOILO1]->tz, value);
				  ...
				  struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (bcl_dev->zone[BATOILO2]->tz) {
		bcl_set_trip0_temperature(bcl_dev->zone[BATOILO2]->tz, value);
			      ...
			      struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (bcl_dev->zone[SMPL_WARN]->tz) {
		bcl_set_trip0_temperature(bcl_dev->zone[SMPL_WARN]->tz,
		       ...
		       u16 llimit, u16 ulimit, u16 step, u8 id)
{
	...
	if (bcl_dev->zone[id]->tz) {
		bcl_set_trip0_temperature(bcl_dev->zone[id]->tz, val);
...
static const struct bin_attribute *const br_stats_bin_attrs[] = {
	&br_stats_dump_attr,
