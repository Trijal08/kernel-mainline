...
/*
 * NOTE(mainline): the in-kernel DMA-heap consumer API (dma_heap_find(),
 * dma_heap_buffer_alloc(), dma_heap_put()) that the Android kernel exported is
 * not present in mainline 7.1 - only the producer side (dma_heap_add()) is.
 * The helpers below reconstruct the consumer side on top of the stable
 * /dev/dma_heap/<name> character-device interface:
 *   - dma_heap_open() stores the "struct dma_heap *" in file->private_data, so
 *     opening the node yields the heap handle while pinning the heap.
 *   - the file reference is tracked separately and released by dma_heap_put().
 *
 * Buffer allocation through this path additionally needs the secure
 * "vframe-secure" heap, which does not exist in mainline yet. Until that heap
 * is ported, dma_heap_buffer_alloc() returns -EOPNOTSUPP so that slab
 * allocation fails cleanly rather than the driver mis-behaving. This path
 * needs review once the secure heap lands.
 */

static struct dma_heap *mali_pma_dma_heap_find(const char *name,
					       struct file **heap_file)
{
	struct file *filp;
...
static struct dma_buf *mali_pma_dma_heap_buffer_alloc(struct dma_heap *heap,
						      size_t len,
						      unsigned int fd_flags,
						      unsigned int heap_flags)
{
	/* NOTE(mainline): requires the not-yet-ported secure heap; see above. */
	return ERR_PTR(-EOPNOTSUPP);
}

static void mali_pma_dma_heap_put(struct file *heap_file)
{
	if (heap_file)
		fput(heap_file);
}
 ...
 * @dma_heap_file: File reference pinning @dma_heap (mainline consumer-API shim).
...
static struct mali_pma_slab *mali_pma_slab_add(
	...
	dma_buf = mali_pma_dma_heap_buffer_alloc(
...
static int protected_memory_allocator_probe(struct platform_device *pdev)
{
	...
	struct file *pma_heap_file = NULL;
	struct mali_pma_dev *mali_pma_dev;
	struct protected_memory_allocator_device *pma_dev;
	int ret = 0;

	/* Try locating a PMA heap, defer if not present (yet). */
	pma_heap = mali_pma_dma_heap_find(MALI_PMA_DMA_HEAP_NAME, &pma_heap_file);
	if (!pma_heap) {
		dev_warn(&(pdev->dev),
	...
	if (!mali_pma_dev) {
		dev_err(&(pdev->dev),
			...
			"device record\n");
		mali_pma_dma_heap_put(pma_heap_file);
		ret = -ENOMEM;
		goto out;
	}
	...
	mali_pma_dev->dma_heap_file = pma_heap_file;
...
static void protected_memory_allocator_remove(struct platform_device *pdev)
{
	...
	mali_pma_dma_heap_put(mali_pma_dev->dma_heap_file);
