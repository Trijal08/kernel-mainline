...
void decon_dump_locked(const struct decon_device *decon, struct drm_printer *p)
{
	...
	if (!p) {
		printer = is_console_enabled() ?
			drm_dbg_printer(NULL, DRM_UT_CORE, "[drm]") :
				 ...
				 struct drm_crtc_state *crtc_state)
{
	...
	if (exynos_crtc_state->planes_updated) {
		decon_info(decon, "%s: planes updated on commit, skipping handover\n", __func__);
		return 0;
	}
	...
	if (!win_mask) {
		decon_warn(decon, "%s: handover memory defined, but no windows attached\n", __func__);
		return -ENOENT;
	}

	if (!found_handover_dpp) {
		decon_warn(decon, "%s: handover memory defined, but cannot find handover dpp\n",
				__func__);
		return -EBUSY;
	}
		...
		const struct exynos_drm_crtc_state *new_exynos_crtc_state)
{
	...
	if (ktime_after(earliest_process_time, now)) {
		...
		DPU_ATRACE_BEGIN("wait for earliest present time (vsync:%d, delay %dus)", te_freq,
				 delay_until_process);
		usleep_range_idle(delay_until_process, delay_until_process + 10);
				...
				const struct drm_crtc_state *new_crtc_state)
{
	...
	/*
	 * NOTE(mainline): the internal wait_for_common() is not exported. Use
	 * the public timeout wait, which returns remaining jiffies (0 on
	 * timeout) -- same condition semantics. The only difference is the wait
	 * runs in TASK_UNINTERRUPTIBLE rather than TASK_IDLE, so it may
	 * contribute to the load average during the wait.
	 */
...
/*
 * NOTE(mainline): iommu_register_device_fault_handler() and its per-device
 * fault-handler callback type were removed upstream in favour of the IOPF/SVA
 * fault-queue infrastructure, which has no drop-in equivalent for this SoC's
 * synchronous SysMMU fault reporting. The handler body is retained (marked
 * __maybe_unused) so it can be re-wired to the new fault path later.
 */
static __maybe_unused int dpu_sysmmu_fault_handler(struct iommu_fault *fault,
						   void *data)
{
...
static int decon_bind(struct device *dev, struct device *master, void *data)
{
	...
	/*
	 * NOTE(mainline): iommu_register_device_fault_handler() was removed
	 * upstream; SysMMU fault reporting via dpu_sysmmu_fault_handler() is
	 * disabled until it can be re-wired to the IOPF/SVA fault path.
	 */
...
#endif
	/* NOTE(mainline): iommu_unregister_device_fault_handler() was removed upstream. */
...
static int decon_probe(struct platform_device *pdev)
{
	...
	/* NOTE(mainline): was SCHED_FIFO prio 20; mapped to the exported
	 * sched_set_fifo() (mid-range RT prio).
	 */
	sched_set_fifo(decon->thread);
