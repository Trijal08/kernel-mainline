...
/*
 * NOTE(mainline): This is debug-only buffer test scaffolding (its sole caller,
 * dpp_test_fixed_config_params(), is __maybe_unused and unreferenced). The
 * downstream consumer-side DMA-BUF heap API (dma_heap_find()/
 * dma_heap_buffer_alloc()/dma_heap_put()) does not exist in mainline, so this
 * test helper is disabled here. Re-implement against a mainline allocator if the
 * fixed-config DPP self-test is ever needed again.
 */
static __maybe_unused dma_addr_t dpp_alloc_map_buf_test(void)
{
	pr_err("%s: DMA-BUF system heap test path unavailable in mainline\n",
	       __func__);
	return (dma_addr_t)-EINVAL;
}
