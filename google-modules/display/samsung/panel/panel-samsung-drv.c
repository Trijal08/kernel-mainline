...
int exynos_panel_set_power(struct exynos_panel *ctx, bool on)
{
	...
	ctx->bl->props.power = on ? BACKLIGHT_POWER_ON : BACKLIGHT_POWER_OFF;
 ...
 */
/*
 * NOTE(mainline): drm_mode_convert_to_umode() is DRM-internal and not exported
 * to modules in mainline. Reimplement a local equivalent that copies the
 * drm_display_mode fields into the uapi drm_mode_modeinfo.
 */
static void exynos_convert_to_umode(struct drm_mode_modeinfo *out,
				    const struct drm_display_mode *mode)
{
	out->clock = mode->clock;
	out->hdisplay = mode->hdisplay;
	out->hsync_start = mode->hsync_start;
	out->hsync_end = mode->hsync_end;
	out->htotal = mode->htotal;
	out->hskew = mode->hskew;
	out->vdisplay = mode->vdisplay;
	out->vsync_start = mode->vsync_start;
	out->vsync_end = mode->vsync_end;
	out->vtotal = mode->vtotal;
	out->vscan = mode->vscan;
	out->vrefresh = drm_mode_vrefresh(mode);
	out->flags = mode->flags;
	out->type = mode->type;
	strscpy(out->name, mode->name, sizeof(out->name));
}
				    ...
				    uint64_t *val)
{
	...
	if (ctx->desc->lp_mode_count <= 1 || !cur_mode) {
		...
		exynos_convert_to_umode(&umode, &ctx->desc->lp_mode->mode);
	} else {
		int i;
		...
		for (i = 0; i < ctx->desc->lp_mode_count; i++) {
			...
			exynos_convert_to_umode(&umode, &lp_mode->mode);
...
static void hbm_data_init(struct exynos_panel *ctx)
{
	...
	if (is_local_hbm_post_enabling_supported(ctx)) {
		...
		if (IS_ERR(ctx->hbm.local_hbm.thread))
			...
			/* NOTE(mainline): was SCHED_FIFO prio 2; mapped to the
			 * exported sched_set_fifo_low() (FIFO prio 1).
			 */
			sched_set_fifo_low(ctx->hbm.local_hbm.thread);
