...
static void dp_on_by_hpd_plug(struct dp_device *dp)
{
	...
	/*
	 * NOTE(mainline): drm_do_get_edid() was removed in favour of the opaque
	 * drm_edid API. Read via drm_edid_read_custom() and copy the raw EDID
	 * (base block plus any extension blocks) into a writable legacy
	 * "struct edid *" so the existing buffer-based flow below is preserved.
	 */
	...
	if (drm_edid) {
		...
		if (raw) {
			size_t len = EDID_LENGTH * (1 + raw->extensions);

			edid = kmemdup(raw, len, GFP_KERNEL);
		}
...
static enum drm_mode_status dp_conn_mode_valid(struct drm_connector *connector, const struct drm_display_mode *mode)
{
	struct dp_device *dp = connector_to_dp(connector);
	struct drm_display_info *display_info = &connector->display_info;
