...
static struct drm_private_state *exynos_atomic_create_priv_state(struct drm_private_obj *obj)
{
	...
	/*
	 * NOTE(mainline): drm_atomic_private_obj_init() no longer takes an
	 * initial state. Instead the core calls this hook to allocate the
	 * pristine private object state, so the initialization that previously
	 * happened at the call site now lives here.
	 */
...
static struct drm_driver exynos_drm_driver = {
	...
	.name			   = DRIVER_NAME,
	.desc			   = DRIVER_DESC,
	/* NOTE(mainline): drm_driver.date was removed upstream. */
	.major			   = DRIVER_MAJOR,
	.minor			   = DRIVER_MINOR,
};
...
static int exynos_drm_bind(struct device *dev)
{
	...
	/*
	 * NOTE(mainline): drm_atomic_private_obj_init() now allocates the
	 * pristine state itself via the .atomic_create_state hook and returns
	 * an error code, so the manual state allocation done here previously is
	 * no longer needed.
	 */
	ret = drm_atomic_private_obj_init(drm, &private->obj,
	...
	/*
	 * NOTE(mainline): drm_device.irq_enabled was removed along with the DRM
	 * legacy IRQ midlayer. This driver supplies its own IRQ handler and uses
	 * KMS vblank directly, so no flag needs to be set here.
	 */
