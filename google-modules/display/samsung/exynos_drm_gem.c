...
/*
 * NOTE(mainline): The downstream consumer-side DMA-BUF heap API
 * (dma_heap_find()/dma_heap_buffer_alloc()/dma_heap_put()) does not exist in
 * mainline; only the provider-side API is exported. To keep GEM buffer
 * allocation working without that Android-only API, allocate a coherent,
 * physically-contiguous buffer against the display IOMMU client device and wrap
 * it in a minimal dma_buf exporter below. This is functionally equivalent to
 * allocating from the system heap for the display pipeline's purposes, but the
 * allocation is contiguous rather than scatter/gather backed -- review if a
 * non-contiguous backing is required.
 */
struct exynos_system_buffer {
	struct device *dev;
	void *vaddr;
	dma_addr_t dma_addr;
	size_t size;
...
static struct sg_table *exynos_sysbuf_map(struct dma_buf_attachment *attach,
					  enum dma_data_direction dir)
{
	struct exynos_system_buffer *buf = attach->dmabuf->priv;
	struct sg_table *sgt;
	int ret;
	...
	/*
	 * The buffer was allocated coherently against buf->dev, which is the
	 * same display IOMMU client device the importer attaches to, so the
	 * coherent DMA address is already valid for that device.
	 */
...
static void exynos_sysbuf_unmap(struct dma_buf_attachment *attach,
...
static void exynos_sysbuf_release(struct dma_buf *dmabuf)
{
	struct exynos_system_buffer *buf = dmabuf->priv;

	dma_free_coherent(buf->dev, buf->size, buf->vaddr, buf->dma_addr);
	kfree(buf);
}

static int exynos_sysbuf_mmap(struct dma_buf *dmabuf, struct vm_area_struct *vma)
{
	struct exynos_system_buffer *buf = dmabuf->priv;

	return dma_mmap_coherent(buf->dev, vma, buf->vaddr, buf->dma_addr,
				 buf->size);
}

static int exynos_sysbuf_vmap(struct dma_buf *dmabuf, struct iosys_map *map)
{
	struct exynos_system_buffer *buf = dmabuf->priv;
...
static const struct dma_buf_ops exynos_system_buf_ops = {
	.map_dma_buf = exynos_sysbuf_map,
	.unmap_dma_buf = exynos_sysbuf_unmap,
	.release = exynos_sysbuf_release,
	.mmap = exynos_sysbuf_mmap,
	.vmap = exynos_sysbuf_vmap,
};

static struct dma_buf *exynos_drm_system_buffer_alloc(struct device *dev,
						      size_t size)
{
	DEFINE_DMA_BUF_EXPORT_INFO(exp_info);
	struct exynos_system_buffer *buf;
	...
	exp_info.ops = &exynos_system_buf_ops;
	exp_info.size = size;
				 ...
				 unsigned int *gem_handle)
{
	...
	dmabuf = exynos_drm_system_buffer_alloc(priv->iommu_client, size);
	if (IS_ERR(dmabuf)) {
		pr_err("Failed to allocate %#zx bytes for GEM buffer\n", size);
				 ...
				 unsigned int handle, uint64_t *offset)
{
	...
	/*
	 * NOTE(mainline): drm_device.struct_mutex was removed. Both
	 * drm_gem_object_lookup() and drm_gem_create_mmap_offset() perform
	 * their own internal locking, so no driver-side lock is needed here.
	 */
