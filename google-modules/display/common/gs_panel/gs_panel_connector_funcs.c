 ...
 */
static void gs_convert_to_umode(struct drm_mode_modeinfo *out,
				...
				const struct gs_drm_connector_state *gs_state, uint64_t *val)
{
	...
	if ((desc->lp_modes && desc->lp_modes->num_modes <= 1) || !cur_mode) {
		...
		gs_convert_to_umode(&umode, &desc->lp_modes->modes[0].mode);
		...
		for (i = 0; i < desc->lp_modes->num_modes; i++) {
			...
			gs_convert_to_umode(&umode, &lp_mode->mode);
