...
/*
 * NOTE(mainline): struct page_frag_cache no longer exposes a `void *va` field;
 * the page virtual address is now packed into the `encoded_page` field together
 * with the pfmemalloc bit and page order. mm/page_frag_cache.c decodes the va as
 * (encoded_page & PAGE_MASK) via a static helper that is not exported, so we
 * replicate that minimal decode here to recover the cached page's base address.
 */
static inline void *cpif_pf_cache_va(const struct page_frag_cache *nc)
{
	return (void *)(nc->encoded_page & PAGE_MASK);
}
...
void cpif_pcie_iommu_reset(struct pktproc_queue *q)
{
	...
	/* NOTE(mainline): page_frag_cache_drain() handles the empty-cache case and
	 * the page/bias accounting that the open-coded __page_frag_cache_drain() did.
	 */
	page_frag_cache_drain(&ioc->pf_cache);
			     ...
			     u32 idx, u32 *map_cnt)
{
	...
	/* Map the last page */
	*map_cnt = 0;
	if (ioc->map_page_va != cpif_pf_cache_va(&ioc->pf_cache)) {
...
set_map:
		...
		ioc->map_page_va = cpif_pf_cache_va(&ioc->pf_cache);
		ioc->map_idx = idx;
		ioc->pf_offset = 0;
	}

	/* Convert an address in accending order */
	addr_asc = cpif_pf_cache_va(&ioc->pf_cache) + ioc->pf_offset;
	 ...
	 */
	if (idx == (q->num_desc - 1)) {
		/* NOTE(mainline): page_frag_cache_drain() drains the cached page and
		 * resets encoded_page to 0, which replaces the old explicit
		 * "drain + va = NULL" sequence.
		 */
		page_frag_cache_drain(&ioc->pf_cache);
	}
