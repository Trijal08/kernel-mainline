...
#else
/*
 * NOTE(mainline): struct reserved_mem no longer carries an fdt_node field and
 * RESERVEDMEM_OF_DECLARE now registers a struct reserved_mem_ops instead of a
 * single setup function. The fdt node is delivered to the ->node_init callback,
 * which we use to read the "rmem_index" property.
 */
static int __init cp_rmem_node_init(unsigned long fdt_node,
				    struct reserved_mem *rmem)
{
	const __be32 *prop;
	int len;
	...
	prop = of_get_flat_dt_prop(fdt_node, "rmem_index", &len);
	if (!prop) {
...
static const struct reserved_mem_ops cp_rmem_ops = {
	.node_init = cp_rmem_node_init,
};

RESERVEDMEM_OF_DECLARE(modem_if, "exynos,modem_if", &cp_rmem_ops);
