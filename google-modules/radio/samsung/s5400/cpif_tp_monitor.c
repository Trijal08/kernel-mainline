...
/*
 * NOTE(mainline): struct rps_map / RPS_MAP_SIZE moved to <net/rps.h> and
 * struct netdev_rx_queue moved to <net/netdev_rx_queue.h> (out of netdevice.h).
 */
#include <net/rps.h>
...
#if IS_ENABLED(CONFIG_CPU_FREQ)
	/* NOTE(mainline): cpufreq_get_policy() was removed; use cpufreq_cpu_get(). */
